...
const fragmentShaderSource = `
  ...
  void main() {
      ...
      for (float i = 1.0; i <= 3.0; i++) {
          float swirl = sin(r * (4.0 + i) - t * 2.0) * (0.2 * i) + noise(uv * 3.0 - t) * 0.2;
          float currentA = a + swirl;

          float numRibbons = 20.0 * i;
          float id = floor(currentA * numRibbons / 6.28318);
          
          float edgeNoise = noise(vec2(r * 5.0, id + i * 10.0)) * 0.3;
          float localA = fract(currentA * numRibbons / 6.28318 + edgeNoise);

          float ribbon = smoothstep(0.1, 0.3, localA) * smoothstep(0.9, 0.7, localA);
          
          float streak = smoothstep(0.2, 1.0, noise(vec2(id * 2.0, r * 15.0 - t * 3.0)));
          ...
          float h = hash(vec2(id, i));
          vec3 layerCol = mix(c1, c2, sin(id * 0.2 + t) * 0.5 + 0.5);
          if(h > 0.3) layerCol = mix(layerCol, c3, cos(id * 0.5 - t) * 0.5 + 0.5);
          if(h > 0.6) layerCol = c4;
          if(h > 0.85) layerCol = c5;

          float intensity = ribbon * (0.3 + 0.7 * streak) / (1.0 + r * 2.0);
          col += layerCol * intensity;
      }

      float coreR = 0.15;
      float coreMask = smoothstep(coreR, coreR + 0.05, r);
      col *= coreMask;
      ...
      float bgBeams = pow(sin(a * 40.0 + t * 2.0) * 0.5 + 0.5, 4.0);
      col += vec3(1.0, 0.5, 0.2) * bgBeams * 0.1 * coreMask * (1.0 - r);
