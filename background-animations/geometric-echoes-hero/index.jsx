...
const shaderData = {
  ...
  fragment: `
    ...
    // Map the recursive, brutalist "Echoing" geometry
    float map(vec3 p) {
        float t = uTime * 0.4;
        
        // Entire structure slowly tumbles through space
        p.xy *= rot(t * 0.2);
        p.xz *= rot(t * 0.1);
        
        float d = 100.0;
        
        // Generate multiple echoing concentric frames
        for(int i = 0; i < 6; i++) {
            float fi = float(i);
            
            // Each frame rotates slightly offset from the others
            vec3 q = p;
            q.xz *= rot(fi * 0.5 + t * 0.2);
            q.xy *= rot(fi * 0.3 - t * 0.1);
            
            // Outer bounding size shrinks per iteration
            float size = 3.5 - fi * 0.5;
            float thickness = 0.05; // Thin, sharp frames
            
            // Construct a hollow box frame
            float boxOuter = sdBox(q, vec3(size));
            float boxInner = sdBox(q, vec3(size - thickness));
            float frame = max(boxOuter, -boxInner);
            
            // Punch massive holes through the 3 axes to leave only the edges (a wireframe box)
            float holeX = sdBox(q, vec3(size * 1.1, size - thickness * 2.0, size - thickness * 2.0));
            float holeY = sdBox(q, vec3(size - thickness * 2.0, size * 1.1, size - thickness * 2.0));
            float holeZ = sdBox(q, vec3(size - thickness * 2.0, size - thickness * 2.0, size * 1.1));
            
            frame = max(frame, -holeX);
            frame = max(frame, -holeY);
            frame = max(frame, -holeZ);
            
            // Combine with the scene
            d = min(d, frame);
        }
        
        // Add a central, pulsating power core
        float core = length(p) - 0.4 + sin(uTime * 4.0) * 0.05;
        d = min(d, core);
    ...
    void main() {
        ...
        for(int i = 0; i < 100; i++) {
            ...
            // Accumulate soft, ambient energy scattering off the inside of the frames
            glow += 0.001 / (0.01 + abs(d)) * exp(-dTotal * 0.25);
            
            if(d < 0.001 || dTotal > 18.0) break;
            dTotal += d * 0.75; // Small steps for sharp geometry
        }
        
        vec3 col = vec3(0.015, 0.01, 0.02); // Ambient dark void
        ...
        if (dTotal < 18.0) {
            ...
            // Brutalist Concrete / Obsidian material
            col = vec3(0.08, 0.08, 0.1) * diff;
            ...
            // Edge highlights (Fresnel)
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 4.0);
            col += vec3(0.0, 0.5, 1.0) * fresnel * 0.6; // Cyan edges
        }
        
        // Blend in the intense volumetric energy
        col += vec3(1.0, 0.2, 0.0) * glow * 1.5; // Red/Orange burning core
        col += vec3(1.0, 0.8, 0.2) * pow(glow * 0.3, 2.0); // White-hot center
...
export const GeometricEchoesHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#030104] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(3, 1, 4, 0.9) 100%)' }} />
