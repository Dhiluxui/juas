...
const shaderData = {
  ...
  fragment: `
    ...
    // Map a complex knotted mobius/torsion object
    float map(vec3 p) {
        // Slowly rotate the entire object in space
        p.xy *= rot(uTime * 0.5);
        p.xz *= rot(uTime * 0.3);
        
        // We will create a Trefoil-like knot by combining twisted tori
        float d = 100.0;
        
        // Three interlocking rings, heavily twisted
        for(int i = 0; i < 3; i++) {
            vec3 q = p;
            
            // Rotate each ring around the center by 120 degrees
            q.xy *= rot(float(i) * 2.09439); // 2pi / 3
            
            // Offset the ring outward
            q.x -= 1.0;
            
            // Twist the actual ring geometry as it goes around
            float angle = atan(q.z, q.x);
            q.xz *= rot(angle * 2.0); // Double twist
            
            // Squashed, sharp torus profile
            float ring = sdTorus(q, vec2(1.5, 0.15));
            
            // Smooth minimum to melt the rings together where they intersect
            d = clamp( 0.5 + 0.5*(d-ring)/0.5, 0.0, 1.0 );
            d = mix(d, ring, d) - 0.5*d*(1.0-d);
        }
    ...
    vec3 getNormal(vec3 p) {
        vec2 e = vec2(0.005, 0.0); // Sharp normals for metal
    ...
    void main() {
        ...
        vec3 ro = vec3(0.0, 0.0, -8.0); // Camera looking at the knot
        ...
        for(int i = 0; i < 80; i++) {
            ...
            // Volumetric light bleeding off the torsion knot
            aura += 0.01 / (0.02 + abs(d));
            
            if(d < 0.005 || dTotal > 15.0) break;
            dTotal += d;
        }
        
        vec3 col = vec3(0.0); // Studio darkness
        
        if (dTotal < 15.0) {
            vec3 n = getNormal(p);
            
            // FLUID CHROME METAL MATERIAL
            ...
            float spec1 = pow(max(dot(ref, l1), 0.0), 64.0);
            float spec2 = pow(max(dot(ref, l2), 0.0), 32.0);
            
            // Dark base metal
            col = vec3(0.02, 0.03, 0.05);
            
            // Intense dual-tone studio lighting hitting the chrome
            col += vec3(0.0, 0.5, 1.0) * diff1 * 0.5; // Cyan light
            col += vec3(1.0, 0.0, 0.5) * diff2 * 0.5; // Magenta light
            
            // Blinding specular reflections
            col += vec3(1.0) * spec1 * 2.0;
            col += vec3(1.0, 0.8, 0.9) * spec2 * 1.5;
            
            // Extreme glassy fresnel edge highlighting
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 3.0);
            col += vec3(1.0) * fresnel * 1.5;
        }
        
        // Add the volumetric aura
        col += vec3(0.2, 0.0, 1.0) * aura * 0.1;
...
export const QuantumTorsionHero = ({ className = '', children, ...props }: any) => (
