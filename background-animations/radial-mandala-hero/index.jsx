...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
      ...
      float r = length(uv);
      float a = atan(uv.y, uv.x) + uTime * 0.2;
      
      float pattern = sin(a * 8.0) * sin(r * 10.0 - uTime);
      float lines = smoothstep(0.4, 0.45, abs(pattern));
      
      vec3 color = mix(vec3(0.2, 0.0, 0.4), vec3(0.0, 0.5, 0.8), r);
      gl_FragColor = vec4(color * lines, 1.0);
    }
  `
};

export const RadialMandalaHero = ({ className = '', children, ...props }: any) => (
