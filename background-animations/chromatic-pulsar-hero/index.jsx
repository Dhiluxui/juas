  ...
  fragment: `
    ...
    void main() {
        ...
        for(int i = 0; i < 90; i++) {
            ...
            // Core Pulsar
            float core = length(p) - 1.0 + sin(t * 5.0) * 0.05; 
            
            // Accretion Disk / Magnetic Field
            vec3 q = p; q.xz *= rot(t); q.xy *= rot(t * 0.5);
            float shell = abs(length(p) - 2.5 + fbm(q * 2.0 - t)) - 0.1;
            
            // Polar Jets
            float jet = length(p.xz) - 0.1 + p.y * p.y * 0.02;
            
            float d = min(core, min(shell, jet));
            
            // Volumetric energy accumulation
            coreGlow += 0.008 / (0.01 + abs(core));
            shellGlow += 0.006 / (0.02 + abs(shell)) * fbm(p * 3.0 - t);
            jetGlow += 0.003 / (0.02 + abs(jet)) * exp(-abs(p.y) * 0.5);
            
            dTotal += max(abs(d) * 0.7, 0.02);
            if(dTotal > 12.0) break;
        }
        
        vec3 col = vec3(0.01, 0.0, 0.02);
        
        // Chromatic Color Palette
        vec3 c1 = 0.5 + 0.5 * cos(uTime * 2.0 + vec3(0, 2, 4));
        vec3 c2 = 0.5 + 0.5 * cos(uTime * 1.5 + vec3(4, 2, 0));
        
        col += vec3(1.0) * coreGlow * 0.15; // Blinding white core
        col += c1 * shellGlow * 1.5;        // Iridescent accretion disk
        col += c2 * jetGlow * 2.5;          // Intense polar jets
...
export const ChromaticPulsarHero = ({ className = '', children, ...props }: any) => (
