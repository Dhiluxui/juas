...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          float v = 0.5 / r + uTime * 2.5;
          
          vec2 tuv = vec2(u, v);
          
          vec2 grid = fract(tuv * vec2(8.0, 16.0));
          
          float lines = smoothstep(0.0, 0.05, grid.x) * smoothstep(1.0, 0.95, grid.x);
          lines *= smoothstep(0.0, 0.05, grid.y) * smoothstep(1.0, 0.95, grid.y);
          
          float streaks = sin(tuv.y * 15.0 - uTime * 10.0) * 0.5 + 0.5;
          streaks *= smoothstep(0.8, 1.0, sin(tuv.x * 25.0));
          
          float pattern = (1.0 - lines) + (streaks * 1.5);
          
          vec3 color = mix(vec3(0.0, 0.4, 1.0), vec3(0.0, 1.0, 0.6), sin(tuv.x * 3.14 + uTime) * 0.5 + 0.5);
          
          float depth = smoothstep(0.0, 0.4, r);
          
          vec3 finalColor = color * pattern * depth;
          
          finalColor += vec3(0.8, 0.9, 1.0) * (0.01 / (r + 0.01));
...
export const HyperspaceTunnelHero = ({ className = '', children, ...props }: any) => (
