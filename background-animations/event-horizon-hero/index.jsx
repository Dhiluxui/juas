...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          float lens = 1.0 / (r * r + 0.05);
          vec2 warpedUv = uv * (1.0 - 0.03 * lens);
          
          float warpedR = length(warpedUv);
          float warpedAngle = atan(warpedUv.y, warpedUv.x);

          float diskRadius = 0.5;
          float turbulence = sin(warpedAngle * 12.0 + uTime * 3.0) * 0.02 +
                             cos(warpedAngle * 25.0 - uTime * 5.0) * 0.01;
                             
          float distToDisk = abs(warpedR - diskRadius + turbulence);
          
          float diskGlow = 0.005 / (distToDisk + 0.001);
          diskGlow *= smoothstep(0.15, 0.35, warpedR);

          float photonRing = 0.001 / (abs(r - 0.15) + 0.0005);
          float singularity = smoothstep(0.14, 0.16, r);

          float stars = fract(sin(dot(warpedUv * 15.0, vec2(12.9898, 78.233))) * 43758.5453);
          stars = step(0.99, stars) * (sin(uTime * 2.0 + warpedR * 20.0) * 0.5 + 0.5);

          vec3 col = vec3(0.0);
          col += vec3(stars);                                    
          col += vec3(1.0, 0.4, 0.0) * diskGlow;                 
          col += vec3(1.0, 0.9, 0.5) * photonRing;               
          col *= singularity;
...
export const EventHorizonHero = ({ className = '', children, ...props }: any) => (
