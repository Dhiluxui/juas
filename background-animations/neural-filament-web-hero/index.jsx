...
const shaderData = {
  ...
  fragment: `
            ...
            void main() {
                ...
                vec3 col = vec3(0.02, 0.02, 0.05); // Deep space background
                
                // Draw filament chains
                for(int i = 0; i < 15; i++) {
                    float fi = float(i);
                    float t = uTime * 0.2 + fi * 0.5;
                    
                    // Procedural organic paths
                    vec2 p1 = vec2(sin(t * 0.5) * 0.5, cos(t * 0.3) * 0.5);
                    vec2 p2 = vec2(cos(t * 0.4) * 0.5, sin(t * 0.6) * 0.5);
                    
                    // Gravitational Mouse interaction
                    float distToMouse = length(uv - mouse);
                    vec2 offset = normalize(mouse - uv) * (0.1 / (distToMouse + 0.1));
                    
                    float d = sdSegment(uv + offset * 0.2, p1, p2);
                    
                    // Bloom/Glow calculation
                    float glow = 0.005 / (d + 0.001);
                    glow *= smoothstep(0.8, 0.0, length(uv)); 
                    
                    // Color dynamics
                    vec3 accent = mix(vec3(0.0, 0.8, 1.0), vec3(0.6, 0.0, 1.0), sin(t) * 0.5 + 0.5);
                    col += glow * accent * 0.2;
                }
...
export const NeuralFilamentWebHero = ({ className = '', children, ...props }: any) => (
