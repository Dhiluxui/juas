  ...
  fragment: `
      ...
      void main() {
          ...
          for(int j = 0; j < 4; j++) {
              ...
              p.x += sin(p.y * 2.0 + a) * 0.5;
              p.y += cos(p.x * 1.5 - a) * 0.4;
              
              float v = abs(sin(p.y * 3.0 + p.x * 2.0));
              
              float ribbon = 0.02 / (v + 0.01);
              
              vec3 ribbonCol = mix(vec3(1.0, 0.2, 0.5), vec3(1.0, 0.8, 0.2), i / 3.0);
              
              col += ribbonCol * ribbon * (1.0 - length(uv) * 0.4);
          }
          
          col += vec3(0.05, 0.01, 0.08) * smoothstep(2.0, 0.0, length(uv));
...
export const EtherealAuroraHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#030005] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(3, 0, 5, 0.8) 100%)' }} />
