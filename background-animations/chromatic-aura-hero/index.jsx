...
const shaderData = {
  ...
  fragment: `
    ...
    // 3D Hash and Value Noise for Volumetric Smoke
    float hash(vec3 p) {
    ...
    // Fractal Brownian Motion (FBM) for complex fluid textures
    float fbm(vec3 p) {
        float f = 0.0;
        float amp = 0.5;
        for(int i = 0; i < 5; i++) {
    ...
    void main() {
        ...
        // Smooth interactive camera orbit
        ...
        for(int i = 0; i < 60; i++) {
            ...
            // Domain Warping: Morph the coordinate space dynamically
            vec3 q = p;
            q.x += sin(p.y * 1.2 + t);
            q.y += cos(p.z * 1.5 + t * 0.8);
            q.z += sin(p.x * 1.1 - t);
            
            // Evaluate density field using FBM
            float density = fbm(q * 1.2 - vec3(0.0, t * 1.5, 0.0));
            
            // Confine the aura to a glowing spherical core region
            float coreDist = length(p);
            float core = 1.0 - smoothstep(0.0, 3.5, coreDist);
            density *= core; // Fade out fluid at the edges
            
            if (density > 0.02) {
                // Iridescent Spectral Coloring based on depth and position
                vec3 spectrum = 0.5 + 0.5 * cos(t + p.y * 2.0 + p.z + vec3(0.0, 2.0, 4.0));
                
                // Add rich depth attenuation
                float att = exp(-dTotal * 0.15);
                
                // Accumulate color
                col += spectrum * density * att * 0.12;
            }
            
            // Step size depends on fluid density (slower inside thick clouds)
            dTotal += max(0.05, 0.02 + density * 0.1);
            if(dTotal > 8.0) break; // Far clipping plane
        }
        
        // Add an ambient background iridescent glow
        vec3 bgSpectrum = 0.5 + 0.5 * cos(t * 0.5 + uv.x + uv.y + vec3(0.0, 1.5, 3.0));
        col += bgSpectrum * 0.08 * smoothstep(1.5, 0.0, length(uv));

        // Depth & Contrast Post-Processing
...
export const ChromaticAuraHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020104] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 20%, rgba(2, 1, 4, 0.9) 100%)' }} />
