...
const shaderData = {
  ...
  fragment: `
    ...
    // Map the massive field of floating hollow crystal matrices
    float map(vec3 p) {
        vec3 q = p;
        
        // 3D Spatial Repetition
        float spacing = 3.5;
        vec3 id = floor(q / spacing);
        q = mod(q, spacing) - spacing * 0.5;
        
        // Each individual cube rotates independently based on its ID and time
        q.xy *= rot(uTime + id.z);
        q.xz *= rot(uTime * 0.8 + id.x);
        
        // The cubes smoothly pulse in size
        float pulse = sin(uTime * 2.0 + length(id)) * 0.2 + 0.6;
        float d = sdBox(q, vec3(pulse));
        
        // Architecturally hollow out the cubes, creating thin, sharp geometric frames
        float inner = sdBox(q, vec3(pulse - 0.1));
        d = max(d, -inner);
    ...
    void main() {
        ...
        // Endless diagonal pan through the floating matrix
        vec3 ro = vec3(uTime, uTime * 0.5, uTime * 0.2);
        ...
        for(int i = 0; i < 90; i++) {
            ...
            // Volumetrically accumulate neon light emitted from the crystal frames
            glow += 0.003 / (0.01 + abs(d)) * exp(-dTotal * 0.15);
            
            if(d < 0.001 || dTotal > 20.0) break;
            
            // Use a slightly conservative step to avoid tunneling through the thin hollow walls
            dTotal += d * 0.7; 
        }
        
        vec3 col = vec3(0.01, 0.01, 0.02); // Deep void background
        
        if (dTotal < 20.0) {
            vec3 n = getNormal(p);
            vec3 l = normalize(vec3(sin(uTime), 1.0, cos(uTime))); // Dynamic light source
            float diff = max(dot(n, l), 0.0);
            
            col = vec3(0.05) * diff; // Very dark, glass-like body
            
            // Intense Neon Fresnel edges that shift color based on position
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 3.0);
            vec3 edgeCol = mix(vec3(0.0, 1.0, 0.8), vec3(1.0, 0.0, 0.8), sin(p.z + uTime) * 0.5 + 0.5);
            col += edgeCol * fresnel * 2.0; 
        }
        
        // Flood the scene with volumetric neon glow
        vec3 neon = mix(vec3(0.8, 0.0, 1.0), vec3(0.0, 1.0, 0.8), sin(uTime + ro.z) * 0.5 + 0.5);
        col += neon * glow * 0.6;
        
        // Very heavy distance fog
        col = mix(col, vec3(0.01, 0.01, 0.02), smoothstep(10.0, 20.0, dTotal));
...
export const CrystalLatticeV2Hero = ({ className = '', children, ...props }: any) => (
