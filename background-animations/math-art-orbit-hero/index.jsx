...
const shaderData = {
  ...
  fragment: `
    ...
    // Mathematical Orbital Map
    float map(vec3 p) {
        float d = 100.0;
        float t = uTime * 0.6;
        
        // Generate multiple interlocking torus shapes representing orbits
        for(int i = 0; i < 4; i++) {
            vec3 q = p;
            float fi = float(i);
            
            // Complex multi-axis rotation simulating gyroscope / orbital mechanics
            q.xy *= rot(t + fi * 1.618);
            q.xz *= rot(t * 1.3 - fi * 0.5);
            q.yz *= rot(t * 0.8 + fi * 2.0);
            
            // Each orbit pulsates slightly in size
            float radius = 1.5 + sin(t + fi) * 0.2;
            float thickness = 0.15 - fi * 0.02;
            
            float torus = sdTorus(q, vec2(radius, thickness));
            d = min(d, torus);
        }
        
        // Add a dense, gravitationally heavy core
        float core = length(p) - 0.3 - sin(t * 5.0) * 0.05;
        d = min(d, core);
    ...
    void main() {
        ...
        for(int i = 0; i < 80; i++) {
            ...
            // Volumetric Gravity Well glow
            glow += 0.003 / (0.01 + abs(length(p) - 0.3));
        ...
        if (dTotal < 15.0) {
            ...
            // Polished Brass / Gold Mathematical Instrument Material
            col = vec3(0.8, 0.6, 0.2) * diff;
            
            // Brilliant specular shines
            vec3 ref = reflect(rd, n);
            float spec = pow(max(dot(ref, l), 0.0), 64.0);
            col += spec * vec3(1.0, 0.9, 0.5) * 2.0;
            
            // Cyan plasma rim lighting (Fresnel)
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 4.0);
            col += vec3(0.0, 0.8, 1.0) * fresnel * 1.5; 
        }
        
        // Orbital Core Energy
        col += vec3(0.0, 0.5, 1.0) * glow * 0.5; // Outer cyan aura
        col += vec3(1.0, 0.2, 0.8) * pow(glow * 0.3, 2.0); // Dense magenta center
...
export const MathArtOrbitHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#010102] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(1, 1, 2, 0.9) 100%)' }} />
