...
const shaderData = {
  ...
  fragment: `
            ...
            // Iterative domain warp for liquid feel
            float fbm(vec2 p) {
                float v = 0.0;
                float a = 0.5;
                for(int i = 0; i < 5; i++) {
            ...
            void main() {
                ...
                // Domain warping: distortion driven by time and position
                ...
                // Coloring based on warping density
                vec3 col = mix(vec3(0.05, 0.02, 0.1), vec3(0.1, 0.3, 0.5), clamp(f * 2.0, 0.0, 1.0));
                col = mix(col, vec3(0.9, 0.9, 1.0), clamp(length(q), 0.0, 1.0) * 0.3);
                col = mix(col, vec3(0.0, 0.1, 0.2), f * 0.5);

                // Add subsurface glow effect
                col += 0.1 * vec3(0.5, 0.7, 1.0) / (0.1 + abs(f - 0.5));
...
export const SubsurfaceFluidWarpHero = ({ className = '', children, ...props }: any) => (
