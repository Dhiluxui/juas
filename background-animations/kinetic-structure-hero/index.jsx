...
const shaderData = {
  ...
  fragment: `
    ...
    // Map the Infinite Kinetic Engine
    float map(vec3 p) {
        float t = uTime;
        ...
        // Massive vertical support pillars
        float pillar = sdCylinder(q, vec3(0.0, 0.0, 0.5));
        
        // Kinetic Spinning Rings around the pillars
        vec3 rq = q;
        rq.y = mod(rq.y + t + p.x * 0.2, 4.0) - 2.0; // Rings slide up and down
        rq.xz *= rot(t * 2.0); // Rings spin rapidly
        
        // Construct a hollow ring
        float ring = max(sdCylinder(rq, vec3(0.0, 0.0, 1.2)), -sdCylinder(rq, vec3(0.0, 0.0, 0.9)));
        ring = max(ring, abs(rq.y) - 0.2); // Cap the height
        
        // Add physical spokes connecting ring to pillar
        float spokes = sdBox(rq, vec3(1.5, 0.1, 0.1));
        float spokes2 = sdBox(rq, vec3(0.1, 0.1, 1.5));
        ring = min(ring, min(spokes, spokes2));
        
        // Horizontal pneumatic pistons that thrust back and forth
        vec3 pq = q;
        pq.y = mod(pq.y + 2.0, 4.0) - 2.0;
        pq.x += sin(t * 3.0 + p.z) * 1.5;
        float piston = sdCylinder(pq.yxz, vec3(0.0, 0.0, 0.3)); // Oriented along X axis
        
        return min(pillar, min(ring, piston));
    }
    ...
    void main() {
        ...
        // Dynamic camera flying diagonally down through the machine
        vec3 ro = vec3(0.0, uTime, -uTime * 2.0); 
        ...
        // Constant slight camera sway for scale
        rd.xy *= rot(sin(uTime*0.2)*0.2); 
        ro.xz *= rot(sin(uTime*0.1)*0.5);
        rd.xz *= rot(sin(uTime*0.1)*0.5);
        ...
        vec3 col = vec3(0.02, 0.02, 0.03); // Dark industrial atmosphere
        ...
        if (dTotal < 30.0) {
            ...
            // Greasy, heavy metallic material
            col = vec3(0.12, 0.15, 0.18) * diff;
            ...
            // Industrial neon orange rim lighting
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 3.0);
            col += vec3(1.0, 0.4, 0.0) * fresnel * 0.8;
        }
        
        // Add the intense friction heat radiating from the moving parts
        col += vec3(1.0, 0.1, 0.0) * glow * 1.5;
        
        // Deep depth-of-field fog
        col = mix(col, vec3(0.01, 0.01, 0.02), smoothstep(10.0, 30.0, dTotal));
...
export const KineticStructureHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#010102] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(1, 1, 2, 0.95) 100%)' }} />
