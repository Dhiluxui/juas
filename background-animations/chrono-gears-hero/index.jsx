  ...
  fragment: `
      ...
      void main() {
          ...
          for(int j = 0; j < 4; j++) {
              ...
              p *= rot(t * (i + 1.0) * direction);
              
              float r = length(p);
              float a = atan(p.y, p.x);
              
              float teethCount = 10.0 + i * 4.0;
              float baseRadius = 0.3 + i * 0.2;
              float dist = r - baseRadius - sin(a * teethCount) * 0.02;
              
              dist = abs(dist) - 0.01;
              
              float glow = 0.005 / (dist * dist + 0.0005);
              
              vec3 gearCol = mix(vec3(0.0, 0.8, 1.0), vec3(1.0, 0.6, 0.1), i / 3.0);
              col += gearCol * glow;
              
              float spokes = abs(sin(a * 4.0 + t * 5.0)) * r;
              float spokeGlow = 0.001 / (spokes + 0.001);
              col += gearCol * spokeGlow * smoothstep(0.0, baseRadius, r);
          }
...
export const ChronoGearsHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#030104] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(3, 1, 4, 0.8) 100%)' }} />
