...
const shaderData = {
  ...
  fragment: `
      ...
      float map(vec3 p) {
          ...
          // Domain repetition for grid
          vec2 id = floor(q.xz / 1.5);
          q.xz = mod(q.xz, 1.5) - 0.75;
          
          // Random height and offset per voxel
          float h = hash(id);
...
export interface VoxelRainHeroProps extends React.HTMLAttributes<HTMLDivElement> {
  className?: string;
  children?: React.ReactNode;
}

export const VoxelRainHero = ({ className = '', children, ...props }: VoxelRainHeroProps) => (
  <div className={`relative w-full h-full bg-[#030107] overflow-hidden font-sans ${className}`} {...props}>
