...
const shaderData = {
  ...
  fragment: `
    ...
    // Map a complex 3D network of vibrating quantum strings
    float map(vec3 p) {
        // Divide space into an infinite 3D grid
        vec3 q = mod(p, 4.0) - 2.0;
        
        // High frequency vibration/plucking math
        float pluckX = sin(p.x * 5.0 + uTime * 20.0) * 0.1;
        float pluckY = sin(p.y * 5.0 + uTime * 25.0) * 0.1;
        float pluckZ = sin(p.z * 5.0 + uTime * 30.0) * 0.1;
        
        // Create 3 intersecting sets of incredibly thin tubes (strings)
        float stringX = length(q.yz + vec2(pluckY, pluckZ)) - 0.01;
        float stringY = length(q.xz + vec2(pluckX, pluckZ)) - 0.01;
        float stringZ = length(q.xy + vec2(pluckX, pluckY)) - 0.01;
        
        // Merge the strings
        return min(stringX, min(stringY, stringZ));
    }
    ...
    void main() {
        ...
        // Camera flying diagonally through the strings
        ...
        // Fast barrel roll for disorientation
        rd.xy *= rot(uTime * 0.5);
        
        float dTotal = 0.0;
        vec3 p;
        
        // Accumulate volumetric light emitting from the vibrating strings
        float stringGlow = 0.0;
        
        for(int i = 0; i < 90; i++) {
            p = ro + rd * dTotal;
            float d = map(p);
            
            // The strings glow intensely, especially where they intersect
            stringGlow += 0.002 / (0.005 + abs(d));
            
            if(d < 0.005 || dTotal > 20.0) break;
            dTotal += d * 0.8;
        }
        
        vec3 col = vec3(0.0); // Pitch black void
        
        if (dTotal < 20.0) {
            vec3 n = getNormal(p);
            
            // The strings themselves are made of pure energy (pure white core)
            col = vec3(1.0);
            
            // Fresnel edge falloff to give them a slightly rounded appearance
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 1.5);
            col *= fresnel;
        }
        
        // Add the massive volumetric aura radiating from the strings
        // Shift colors across the spectrum based on 3D position
        vec3 glowColor = vec3(
            sin(p.x * 0.5) * 0.5 + 0.5,
            sin(p.y * 0.5) * 0.5 + 0.5,
            sin(p.z * 0.5) * 0.5 + 0.5
        ...
        // Boost vibrancy
        glowColor = mix(glowColor, vec3(1.0), 0.2);
        
        col += glowColor * stringGlow * 1.5;
        
        // Heavy distance fog to create depth
...
export const QuantumStringsHero = ({ className = '', children, ...props }: any) => (
