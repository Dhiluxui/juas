...
const shaderData = {
  ...
  fragment: `
    ...
    // Map a massive field of floating glass monoliths
    float map(vec3 p) {
        vec3 q = p;
        
        // Animate the entire field
        q.y += sin(uTime * 0.5 + q.x * 0.2) * 2.0;
        ...
        // Shape: An elongated, sharp octahedron (crystal shard)
        vec3 scale = vec3(0.5, 4.0, 0.5);
        float d = sdOctahedron(q / scale, 1.0) * min(scale.x, min(scale.y, scale.z));
    ...
    // Abstract quantum background field for the glass to reflect/refract
    vec3 background(vec3 rd) {
        float f = sin(rd.x * 10.0 + uTime) * sin(rd.y * 10.0 + uTime) * sin(rd.z * 10.0 + uTime);
        vec3 col = mix(vec3(0.0, 0.2, 1.0), vec3(1.0, 0.0, 0.5), rd.y * 0.5 + 0.5);
        return col * abs(f) * 0.5;
    }

    void main() {
        vec2 uv = (gl_FragCoord.xy - 0.5 * uResolution.xy) / min(uResolution.y, uResolution.x);
        
        vec3 ro = vec3(0.0, 0.0, -10.0 + uTime * 2.0); // Flying forward
        ...
        vec3 col = background(rd); // Default to background
        
        if (dTotal < 40.0) {
            vec3 n = getNormal(p);
            
            // GLASS REFRACTION ALGORITHM
            // Calculate an internal reflection vector
            vec3 ref = reflect(rd, n);
            
            // Chromatic aberration (fake dispersion) by refracting different colors at different indices
            vec3 refrR = refract(rd, n, 0.85);
            vec3 refrG = refract(rd, n, 0.90);
            vec3 refrB = refract(rd, n, 0.95);
            
            // Sample the background using the refracted rays
            float glassDensity = 0.5;
            vec3 glassCol;
            glassCol.r = background(refrR).r;
            glassCol.g = background(refrG).g;
            glassCol.b = background(refrB).b;
            
            // Boost the glass brightness and add extreme specular
            ...
            col = glassCol * 2.0; 
            col += vec3(1.0) * spec * 2.0; // Hot spots
            col += background(ref) * fresnel * 1.5; // Environment reflection on edges
            
            // Fog based on distance
            col = mix(col, background(rd), smoothstep(20.0, 40.0, dTotal));
        }
...
export const AdvancedQuantumGlassHero = ({ className = '', children, ...props }: any) => (
