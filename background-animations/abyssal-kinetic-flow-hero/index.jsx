...
const shaderData = {
  ...
  fragment: `
    ...
    // Kinetic Fluid Logic - Using domain distortion for liquid motion
    void main() {
        ...
        float mouseForce = 1.0 / (1.0 + dist * 5.0);
        ...
        for(int i = 0; i < 3; i++) {
            float s = float(i) * 0.3;
            p.x += 0.6 * sin(p.y + t + s) + mouseForce * 0.5;
            p.y += 0.6 * cos(p.x + t + s) + mouseForce * 0.5;
        }

        // Coloring
        vec3 col = vec3(0.1, 0.1, 0.2); // Base deep void
        col += 0.5 + 0.5 * cos(3.0 + p.xyx + vec3(0.0, 2.0, 4.0));
        col *= 0.5;
        
        // Add pulse
        col += 0.2 * sin(p.y * 2.0 + t);
...
export const AbyssalKineticFlowHero = ({ className = '', children, ...props }: any) => (
