  ...
  fragment: `
    ...
    float map(vec3 p) {
        ...
        for(int j = 0; j < 4; j++) {
            ...
            q.x += sin(angle) * 0.8;
            q.y += cos(angle * 1.2) * 0.8;
            q.z += sin(angle * 0.7) * 0.5;
            d = smin(d, length(q) - 0.4, 0.6);
        }
        return d;
    }
    ...
    void main() {
        ...
        for(int i = 0; i < 64; i++) {
            p = ro + rd * dTotal;
            float d = map(p);
            glow += 0.04 / (0.05 + d * d * 5.0);
            dTotal += d * 0.5;
            if(d < 0.001 || dTotal > 10.0) break;
        }
        
        vec3 col = vec3(0.05, 0.02, 0.1); 
        vec3 glowCol = mix(vec3(0.2, 0.6, 1.0), vec3(1.0, 0.3, 0.8), sin(uTime) * 0.5 + 0.5);
...
export const LuminescentFluidicGlassHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#050508] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(5, 5, 8, 0.8) 100%)' }} />
