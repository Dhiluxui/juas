...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
      ...
      for(int j = 1; j < 4; j++) {
          float i = float(j);
          p.x += 0.3 / i * sin(i * 3.0 * p.y + t) + 0.3;
          p.y += 0.3 / i * cos(i * 3.0 * p.x + t) + 0.3;
      }
      
      float intensity = abs(1.0 / (sin(p.x + p.y) * 10.0));
      vec3 col = vec3(0.1, 0.4, 0.6) * intensity;
      col += vec3(0.2, 0.5, 0.8) * intensity * 0.5;
...
export const CausticDepthsHero = ({ className = '', children, ...props }: any) => (
