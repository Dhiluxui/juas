...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
      vec3 p = vec3((gl_FragCoord.xy * 2.0 - uResolution.xy) / uResolution.y * 3.0, 0.0);
      
      float val = sin(p.x + uTime) * cos(p.y) + sin(p.y + uTime) * cos(p.z) + sin(p.z + uTime) * cos(p.x);
      
      float edge = smoothstep(0.0, 0.1, abs(val) - 0.5);
      vec3 col = vec3(0.9, 0.9, 1.0) * (1.0 - edge);
      col += vec3(0.8, 0.2, 0.5) * edge;
...
export const GyroidSurfaceHero = ({ className = '', children, ...props }: any) => (
