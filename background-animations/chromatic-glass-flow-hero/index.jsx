...
const shaderData = {
  ...
  fragment: `
    ...
    // Simple harmonic noise function for surface undulation
    float noise(vec2 p) {
        return sin(p.x * 2.0 + iTime * 0.5) * sin(p.y * 2.0 + iTime * 0.3) * 0.5 + 0.5;
    }

    // Custom layered FBM-like height function
    float heightMap(vec2 uv) {
        float h = 0.0;
        h += noise(uv * 1.5) * 0.5;
        h += noise(uv * 3.0 + iTime * 0.1) * 0.25;
        h += noise(uv * 6.0 - iTime * 0.2) * 0.125;
        return h;
    }
    ...
    vec3 getNormal(vec2 uv) {
        ...
        return normalize(vec3(h - hx, h - hy, 0.1));
    }

    void main() {
        vec2 uv = (gl_FragCoord.xy - 0.5 * iResolution.xy) / min(iResolution.y, iResolution.x);
        
        // Mouse interaction: Pushing the liquid surface
        ...
        // Surface properties
        vec3 normal = getNormal(uv * 2.0);
        
        // Color Palette: Deep obsidian to metallic gold/teal
        vec3 baseCol = vec3(0.02, 0.02, 0.05); // Midnight blue
        vec3 glowCol = vec3(0.2, 0.8, 1.0);    // Electric cyan
        vec3 goldCol = vec3(1.0, 0.7, 0.3);    // Warm highlight
        
        // Refraction/Highlight Calculation
        vec3 lightDir = normalize(vec3(0.5, 0.5, 1.0));
        float diff = max(dot(normal, lightDir), 0.0);
        float spec = pow(max(dot(reflect(-lightDir, normal), vec3(0.0, 0.0, 1.0)), 0.0), 32.0);
        
        // Fresnal effect for glass realism
        float fresnel = pow(1.0 - max(dot(normal, vec3(0.0, 0.0, 1.0)), 0.0), 3.0);
        
        // Compose final color
        vec3 col = baseCol;
        col = mix(col, glowCol, diff * 0.5);
        col += spec * 0.8;
        col += fresnel * 0.5;
        
        // Vignette
        float v = 1.0 - length(uv) * 0.5;
        col *= v;
...
export const ChromaticGlassFlowHero = ({ className = '', children, ...props }: any) => (
