...
const shaderData = {
  ...
  fragment: `
    ...
    float map(vec3 p) {
        ...
        q.xz *= rot(uTime * 0.15 + q.y * 0.2);
        q.xy *= rot(uTime * 0.1 - q.z * 0.1);

        float disp = snoise(q * 1.5 - vec3(0.0, uTime * 0.4, 0.0)) * 0.6;
        disp += snoise(q * 3.0 + vec3(uTime * 0.2, 0.0, 0.0)) * 0.2;
    ...
    vec3 getEnvironment(vec3 dir) {
        ...
        float p1 = sin(dir.x * 5.0 + t) * cos(dir.y * 5.0 - t);
        float p2 = sin(dir.z * 8.0 - t) * cos(dir.x * 8.0 + t);
        
        vec3 neonPink = vec3(1.0, 0.0, 0.5);
        vec3 neonCyan = vec3(0.0, 0.8, 1.0);
        vec3 neonBlue = vec3(0.1, 0.3, 1.0);
        
        col = mix(col, neonPink, smoothstep(0.0, 1.0, p1) * 0.8);
        col = mix(col, neonCyan, smoothstep(0.0, 1.0, p2) * 0.8);
        
        float sun = max(0.0, dot(dir, normalize(vec3(0.8, 0.5, 1.0))));
        col += vec3(1.0, 0.9, 0.8) * pow(sun, 64.0) * 2.0; 
        col += vec3(0.5, 0.3, 1.0) * pow(sun, 8.0) * 0.5;  
    ...
    void main() {
        ...
        ro.yz *= rot(mouse.y * 0.5);
        rd.yz *= rot(mouse.y * 0.5);
        ro.xz *= rot(-mouse.x * 0.5);
        rd.xz *= rot(-mouse.x * 0.5);
        ...
        if(d0 < 10.0) {
            ...
            vec3 env = getEnvironment(ref);
            
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 3.0);
            
            vec3 baseMetal = vec3(0.1);
            color = baseMetal + env * mix(0.4, 1.0, fresnel);
            
            float r = getEnvironment(reflect(rd, n + vec3(0.02,0,0))).r;
            float b = getEnvironment(reflect(rd, n - vec3(0.02,0,0))).b;
            color.r = mix(color.r, r, 0.5);
            color.b = mix(color.b, b, 0.5);
        }
        
        float glow = smoothstep(0.0, 80.0, iter);
        color += vec3(0.1, 0.0, 0.3) * glow * 1.5;
...
export const QuantumLiquidHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020204] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(2, 2, 4, 0.8) 100%)' }} />
