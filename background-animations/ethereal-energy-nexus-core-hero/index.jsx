...
const shaderData = {
  ...
  fragment: `
    ...
    // Recursive box folding SDF (The "Crystal" structure)
    float map(vec3 p) {
        p.xz *= rot(uTime * 0.1);
        p.xy *= rot(uTime * 0.05);
    ...
    void main() {
        ...
        vec3 ro = vec3(0.0, 0.0, -4.0); // Camera
        vec3 rd = normalize(vec3(uv + mouse * 0.1, 1.5));
        ...
        if(t < 10.0) {
            ...
            col = vec3(0.1, 0.5, 0.7) * diff; // Base structural color
            col += vec3(0.6, 0.1, 0.8) * pow(max(dot(reflect(-lightDir, n), -rd), 0.0), 32.0); // Specular highlight
        }
...
export const EtherealEnergyNexusCoreHero = ({ className = '', children, ...props }: any) => (
