...
const shaderData = {
  ...
  fragment: `
    ...
    // Map a shattered planetary sphere
    float map(vec3 p) {
        ...
        // Create deep, sharp cracks using FBM noise applied to polar coordinates
        // We project the 3D position into a highly chaotic noise field
        float cracks = fbm(p * 5.0) - 0.4;
        
        // Add a secondary fracture scale for micro-details
        float microCracks = fbm(p * 15.0) - 0.5;
        
        // Boolean subtraction: Cut the cracks out of the sphere
        // We use absolute value to create sharp valley folds
        float fracture = sphere + abs(cracks) * 1.5 + abs(microCracks) * 0.2;
        
        // We also push the pieces apart based on the noise (explosion effect)
        // By subtracting the noise from the distance field, pieces expand outward
        fracture -= cracks * sin(uTime * 0.5) * 0.5;
    ...
    void main() {
        ...
        vec3 ro = vec3(0.0, 0.0, -5.0); // Camera looking at the shattered sphere
        ...
        // Accumulator for the intense quantum core energy bleeding through the cracks
        ...
        for(int i = 0; i < 90; i++) {
            ...
            // Check how close we are to the singularity in the dead center
            float distToCore = length(p);
            
            // If the ray penetrates deep into a crack, it accumulates blinding energy
            if (d > 0.0 && distToCore < 1.8) {
                 coreEnergy += 0.02 / (0.05 + d) * exp(-distToCore * 2.0);
            }
            
            if(d < 0.005 || dTotal > 10.0) break;
            dTotal += d * 0.6; // Small step size for high detail fractures
        }
        ...
        if (dTotal < 10.0) {
            ...
            // Dark, rocky obsidian material for the broken shell
            col = vec3(0.02);
            ...
            col += vec3(0.2, 0.2, 0.3) * diff; // Pale moonlight
            col += vec3(1.0) * spec * 0.3; // Slight shine on the rocks
            
            // Highlight the sharp edges of the fractures
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 3.0);
            col += vec3(0.0, 0.5, 1.0) * fresnel * 0.5;
        }
        
        // Add the intense, blinding quantum fire radiating from inside the sphere
        vec3 fireColor = mix(vec3(1.0, 0.0, 0.5), vec3(0.0, 1.0, 1.0), sin(uTime) * 0.5 + 0.5);
        col += fireColor * coreEnergy * 2.0;
...
export const QuantumFractureHero = ({ className = '', children, ...props }: any) => (
