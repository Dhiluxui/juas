...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
        ...
        // Pushing slowly into the nebula
        vec3 ro = vec3(0.0, 0.0, uTime * 0.5);
        ...
        // Heavy Volumetric Cloud Raymarching
        for(int i = 0; i < 70; i++) {
            vec3 p = ro + rd * dTotal;
            
            // Calculate cloud density at this point in space
            float den = fbm(p * 0.5);
            den = smoothstep(0.4, 0.8, den); // Threshold to carve out distinct, fluffy cloud shapes
            
            if (den > 0.01) {
                // Ethereal Blue to Gold color mapping based on space and time
                vec3 cCol = mix(vec3(0.0, 0.3, 0.8), vec3(1.0, 0.6, 0.0), fbm(p * 1.5 + uTime * 0.2));
                
                // Directional Lighting Fake:
                // Sample the density slightly further along the light vector to create self-shadowing
                vec3 lightDir = normalize(vec3(1.0, 1.0, -1.0));
                float shadowDen = fbm(p * 0.5 + lightDir * 0.5);
                float dif = clamp((den - shadowDen) * 3.0, 0.0, 1.0);
                
                // Add the light color to the illuminated parts of the cloud
                cCol += vec3(1.0, 0.9, 0.7) * dif * 1.5;
                
                // Depth attenuation (clouds further away are darker)
                float att = exp(-dTotal * 0.15);
                
                // Accumulate the final color and density
                col += cCol * den * att * 0.1;
            }
            
            // Step size dynamically adjusts based on cloud density (faster in empty space)
            dTotal += max(0.1, 0.05 + den * 0.2);
            if(dTotal > 20.0) break;
        }
        
        // Render a dense, distant starfield
        float stars = pow(hash(rd * 200.0), 100.0);
...
export const NebulaHero = ({ className = '', children, ...props }: any) => (
