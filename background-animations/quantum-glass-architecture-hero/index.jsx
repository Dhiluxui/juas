...
const shaderData = {
  ...
  fragment: `
    ...
    // A massive, Escher-like infinite glass structure
    float map(vec3 p) {
        // Space folding
        vec3 q = p;
        ...
        // Rotate the architectural blocks
        q.xy *= rot(uTime * 0.2);
        q.yz *= rot(uTime * 0.15);
        
        // Boolean operations to create complex architectural shapes
        float box1 = sdBox(q, vec3(2.0, 0.2, 2.0));
        float box2 = sdBox(q, vec3(0.2, 2.0, 2.0));
        float cyl = sdCylinder(q, vec3(0.0, 0.0, 1.5));
        
        // A box with a hole cut in it, intersected with another box
        float structure = min(box1, box2);
        structure = max(structure, -cyl); // Cut hole
        
        // Add floating spheres inside the holes
        float sphere = length(q) - 0.8;
        
        return min(structure, sphere);
    }
    ...
    // Volumetric nebula background to refract
    vec3 background(vec3 rd) {
        float f = sin(rd.x * 5.0 + uTime) * cos(rd.y * 5.0 + uTime) * sin(rd.z * 5.0);
        vec3 col = mix(vec3(0.0, 0.8, 1.0), vec3(0.8, 0.0, 1.0), rd.y * 0.5 + 0.5);
        return col * abs(f);
    }
    ...
    void main() {
        ...
        vec3 ro = vec3(uTime, uTime * 0.5, uTime * 1.5); // Diagonal flight
        ...
        rd.xy *= rot(sin(uTime * 0.1) * 0.2); // Gentle roll
        ...
        vec3 col = background(rd) * 0.2; // Dim background
        ...
        if (dTotal < 30.0) {
            ...
            // Calculate chromatic dispersion by refracting red, green, and blue slightly differently
            vec3 refrR = refract(rd, n, 0.90);
            vec3 refrG = refract(rd, n, 0.92);
            vec3 refrB = refract(rd, n, 0.94);
            
            vec3 glassCol;
            glassCol.r = background(refrR).r;
            glassCol.g = background(refrG).g;
            glassCol.b = background(refrB).b;
            
            // Fresnel rim lighting
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 3.0);
            
            // Specular highlights from a fake light source
            vec3 l = normalize(vec3(1.0, 2.0, -1.0));
            float spec = pow(max(dot(ref, l), 0.0), 128.0);
            
            col = glassCol * 1.5; // Multiply refracted background
            col += background(ref) * fresnel * 2.0; // Edges reflect the environment heavily
            col += vec3(1.0) * spec * 2.0; // Blinding specular hits
            
            // Distance fog
            col = mix(col, background(rd) * 0.2, smoothstep(15.0, 30.0, dTotal));
        }
...
export const QuantumGlassArchitectureHero = ({ className = '', children, ...props }: any) => (
