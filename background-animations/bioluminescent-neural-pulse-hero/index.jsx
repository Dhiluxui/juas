...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
        ...
        uv *= 1.0 + (0.2 * exp(-distToMouse * 2.0)); 
        ...
        for(int j = 0; j < 4; j++) {
            float i = float(j);
            p.x += sin(p.y * 2.0 + t + i) * 0.5;
            p.y += cos(p.x * 2.0 + t + i) * 0.5;
            
            float d = length(p);
            
            vec3 col = palette(length(uv) + i * 0.1 + t * 0.1);
            
            finalColor += col * (0.01 / abs(d - 0.5));
        }
...
export const BioluminescentNeuralPulseHero = ({ className = '', children, ...props }: any) => (
