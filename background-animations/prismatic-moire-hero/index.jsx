...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          vec2 uvR = uv * rot(sin(uTime * 0.2) * 0.02);
          float patternR = sin(uvR.x * scale) * cos(uvR.y * scale + uTime);
          patternR *= sin((uvR.x - uvR.y) * scale * 0.8 - uTime);
          
          vec2 uvG = uv * rot(sin(uTime * 0.25) * 0.04);
          float patternG = sin(uvG.x * scale) * cos(uvG.y * scale + uTime * 1.1);
          patternG *= sin((uvG.x - uvG.y) * scale * 0.82 - uTime * 1.1);
          
          vec2 uvB = uv * rot(sin(uTime * 0.3) * 0.06);
          float patternB = sin(uvB.x * scale) * cos(uvB.y * scale + uTime * 1.2);
          patternB *= sin((uvB.x - uvB.y) * scale * 0.84 - uTime * 1.2);
          ...
          float shadow = length(uv) * 0.5 + sin(uv.x * 2.0 + uTime) * 0.2;
          col *= smoothstep(0.8, 0.2, shadow);
...
export const PrismaticMoireHero = ({ className = '', children, ...props }: any) => (
