...
const shaderData = {
  ...
  fragment: `
    ...
    // Map a 3D Synthwave/Cyberpunk grid network
    float map(vec3 p) {
        // Divide space into 4x4x4 blocks
        vec3 q = mod(p, 4.0) - 2.0;
        
        // Place a central data node (sphere) in each block
        float node = length(q) - 0.4;
        
        // Shoot laser connecting lines along the X, Y, and Z axes
        float lineX = length(q.yz) - 0.05;
        float lineY = length(q.xz) - 0.05;
        float lineZ = length(q.xy) - 0.05;
        
        // Merge them all into a single continuous grid structure
        float grid = min(node, min(lineX, min(lineY, lineZ)));
    ...
    void main() {
        ...
        // Fly forward and up through the grid diagonally
        vec3 ro = vec3(uTime, uTime * 0.5, uTime * 2.0);
        ...
        // Barrel roll
        rd.xy *= rot(sin(uTime * 0.2) * 0.5);
        
        float dTotal = 0.0;
        vec3 p;
        
        // Volumetric glow accumulator
        float energyGlow = 0.0;
        
        for(int i = 0; i < 90; i++) {
            p = ro + rd * dTotal;
            float d = map(p);
            
            // Volumetrically accumulate light around the grid structure
            // Making it glow intensely when we pass near it
            energyGlow += 0.005 / (0.01 + abs(d)) * exp(-dTotal * 0.05);
            
            if(d < 0.005 || dTotal > 40.0) break;
            dTotal += d;
        }
        
        vec3 col = vec3(0.0); // Black space
        
        if (dTotal < 40.0) {
            vec3 n = getNormal(p);
            
            // Physical material of the grid is dark glass/metal
            col = vec3(0.02, 0.01, 0.03);
            
            // Create a traveling pulse effect based on grid coordinates and time
            vec3 id = floor(p / 4.0);
            float pulse = sin(id.x * 2.0 + id.y + id.z - uTime * 5.0) * 0.5 + 0.5;
            
            // Synthwave colors
            vec3 neonColor = mix(vec3(1.0, 0.0, 0.5), vec3(0.0, 1.0, 1.0), pulse);
            
            // Fresnel rim lighting
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 2.0);
            col += neonColor * fresnel * (1.0 + pulse * 2.0);
        }
        
        // Globally add the intense volumetric aura
        // Base color shifts between magenta and cyan based on position
        vec3 auraBase = mix(vec3(1.0, 0.0, 0.8), vec3(0.0, 0.8, 1.0), sin(p.z * 0.1) * 0.5 + 0.5);
        col += auraBase * energyGlow * 0.8;
        
        // Fade to black distance fog
        col = mix(col, vec3(0.0), smoothstep(20.0, 40.0, dTotal));
...
export const PulsarGridHero = ({ className = '', children, ...props }: any) => (
