...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          vec2 mapUv = vec2(p.x / p.y, 1.0 / p.y);
          
          mapUv.y -= uTime * 0.8;
          mapUv *= 1.5; 
          
          float elevation = terrainHeight(mapUv);
          float contourMap = fract(elevation * 12.0);
          
          float lines = smoothstep(0.0, 0.05, contourMap) * smoothstep(1.0, 0.95, contourMap);
          lines = 1.0 - lines;
          
          float depthFade = smoothstep(0.01, 0.4, p.y);
          lines *= depthFade;

          vec3 lineColor = vec3(0.0, 1.0, 0.3) * lines;
          vec3 valleyColor = vec3(0.0, 0.2, 0.1) * (1.0 - elevation) * depthFade;

          vec3 finalColor = lineColor + valleyColor;
...
export const HolographicTopographyHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#000502] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(0, 5, 2, 0.8) 100%)' }} />
