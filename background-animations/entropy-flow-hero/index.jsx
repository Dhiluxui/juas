...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
      ...
      vec2 q = vec2(0.0);
      q.x = sin(uv.x + uTime * 0.1);
      q.y = cos(uv.y + uTime * 0.1);
      
      vec2 r = vec2(0.0);
      r.x = sin(uv.x + q.x + uTime * 0.2);
      r.y = cos(uv.y + q.y + uTime * 0.3);
      
      float intensity = length(r);
      gl_FragColor = vec4(intensity * 0.2, intensity * 0.4, intensity * 0.6, 1.0);
    }
  `
};

export const EntropyFlowHero = ({ className = '', children, ...props }: any) => (
