...
const shaderData = {
  ...
  fragment: `
    ...
    // Simplex noise for the accretion disk
    ...
    void main() {
        ...
        vec3 ro = vec3(0.0, 1.0, -6.0); // Camera above and back
        ...
        // NON-LINEAR RAYTRACING (GRAVITY LENSING)
        vec3 p = ro;
        vec3 col = vec3(0.0);
        ...
        for(int i = 0; i < 100; i++) {
            ...
            // Event Horizon check
            if (r < 1.0) {
                // Ray fell into the black hole, absolute black.
                break;
            }
            
            // Calculate gravitational pull vector
            // Gravity falls off by r^2. Force pulls the ray toward the origin.
            vec3 gravity = -p / (r * r * r) * GM;
            
            // Bend the ray direction
            rd = normalize(rd + gravity * 0.05); // 0.05 is the integration step size
            
            // Advance the ray
            p += rd * 0.1;
            
            // ACCRETION DISK (Volumetric Torus / Plane)
            // The disk exists on the XZ plane (where y is near 0)
            float distToDisk = abs(p.y);
            
            // Only evaluate disk noise if we are close to the plane
            if (distToDisk < 0.2 && r > 1.2 && r < 5.0) {
                // Swirling polar coordinates for the disk
                float angle = atan(p.z, p.x);
                float swirl = angle - uTime * (3.0 / r); // Inner spins faster (Keplerian)
                
                vec3 noiseCoord = vec3(r * 4.0, swirl * 4.0, p.y * 10.0);
                float matter = fbm(noiseCoord);
                
                // Density falls off with distance from center and from the plane
                float d = smoothstep(0.0, 1.0, matter) * (0.2 - distToDisk) * 5.0;
                
                if (d > 0.0) {
                    // Accumulate light energy from the superheated gas
                    // Shifts from red (outer) to blinding blue/white (inner)
                    vec3 gasColor = mix(vec3(1.0, 0.2, 0.0), vec3(0.5, 0.8, 1.0), smoothstep(4.0, 1.2, r));
                    col += gasColor * d * 0.15 * (1.0 / r); // Brighter near the center
                }
            }
            
            if (r > 15.0) {
                // Ray escaped into deep space
                // Render distant background stars, but severely bent by the lensing
        ...
        // Tonemapping for blinding hot plasma
        col = 1.0 - exp(-col * 2.0);
...
export const CelestialWarpHero = ({ className = '', children, ...props }: any) => (
