...
const shaderData = {
  ...
  fragment: `
    ...
    float lattice(vec2 p) {
        ...
        float amp = 0.5;
        for (int i = 0; i < 4; i++) {
            p += sin(p.yx * 2.0 + uTime * 0.2) * 0.5;
            p *= rot(0.8);
            v += abs(sin(p.x) * cos(p.y)) * amp;
            p *= 2.1;
            amp *= 0.5;
        }
        return v;
    ...
    void main() {
        ...
        float f = lattice(uv * 3.0);
        
        vec3 colA = vec3(0.02, 0.05, 0.1);   
        vec3 colB = vec3(0.0, 0.6, 0.7);     
        vec3 colC = vec3(0.8, 0.2, 0.5);     
        
        vec3 color = mix(colA, colB, f);
        color = mix(color, colC, pow(f, 4.0) * 0.5);
        
        color += vec3(0.1, 0.2, 0.3) * pow(f, 6.0);
...
export const QuantumCrystallineHero = ({ className = '', children, ...props }: any) => (
