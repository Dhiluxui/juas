...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
      vec2 uv = (gl_FragCoord.xy * 2.0 - uResolution.xy) / uResolution.y * 7.5;
      
      vec2 p = fract(uv) - 0.5;
      
      float d = length(p) - 0.2;
      float angle = atan(p.y, p.x) + uTime * 0.5;
      d = abs(d) + sin(angle * 3.0) * 0.05;
      
      float lattice = smoothstep(0.02, 0.0, d - 0.1);
      
      vec3 color = vec3(0.1, 0.2, 0.3) * (1.0 - lattice);
      color += vec3(0.9, 0.9, 1.0) * lattice;
...
export const CrystalLatticeHero = ({ className = '', children, ...props }: any) => (
