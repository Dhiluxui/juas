  ...
  fragment: `
      ...
      void main() {
          ...
          for(int j = 1; j <= 5; j++) {
              ...
              float rad = 0.4 + sin(t * 0.3 + i) * 0.2;
              
              vec2 pos = vec2(sin(phase), cos(phase * 0.8 + i)) * rad;
              float dist = length(uv - pos);
              
              potential += (0.1 * i) / (dist + 0.05); 
          }
          
          float lines = sin(potential * 25.0 - uTime * 5.0);
          float crispLines = smoothstep(0.85, 0.95, lines);
          
          vec3 col = mix(vec3(0.5, 0.0, 1.0), vec3(0.0, 1.0, 0.8), clamp(potential * 0.2, 0.0, 1.0));
          
          col *= crispLines;
          col += vec3(0.0, 0.4, 0.8) * (potential * potential * 0.02);
          col *= smoothstep(2.5, 0.5, length(uv));
...
export const OrbitalGravityHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020108] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(2, 1, 8, 0.8) 100%)' }} />
