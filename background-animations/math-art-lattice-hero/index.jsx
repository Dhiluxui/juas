...
const shaderData = {
  ...
  fragment: `
    ...
    // Distance field for an exact mathematical Menger Sponge Fractal
    float map(vec3 p) {
        float d = sdBox(p, vec3(2.0)); // Start with a massive base cube
        
        float s = 1.0;
        
        // 4 Iterations of fractal space folding
        for(int m = 0; m < 4; m++) {
            // Fold space
            ...
            // Generate cross-shaped holes
            float da = max(r.x, r.y);
            float db = max(r.y, r.z);
            float dc = max(r.z, r.x);
            
            // Subtract holes from the current geometry
    ...
    void main() {
        ...
        // Camera positioned outside looking at the fractal
        ...
        // Complex mathematical tumble
        ro.xz *= rot(uTime * 0.2); rd.xz *= rot(uTime * 0.2);
        ro.xy *= rot(uTime * 0.15); rd.xy *= rot(uTime * 0.15);
        
        float dTotal = 0.0;
        vec3 p;
        
        // High precision raymarch for the fractal
        for(int i = 0; i < 100; i++) {
            p = ro + rd * dTotal;
            float d = map(p);
        ...
        if (dTotal < 12.0) {
            ...
            // Mathematical Iridescent Material mapping spatial coordinates to color
            vec3 objCol = 0.5 + 0.5 * cos(uTime + p.xyx * 2.0 + vec3(0, 2, 4));
            col = objCol * diff * 1.5;
            
            // Fake Ambient Occlusion to simulate deep shadows inside the fractal holes
            float ao = clamp(map(p + n * 0.1) * 10.0, 0.0, 1.0);
            col *= ao;
            
            // Crisp specular edge highlights
            vec3 ref = reflect(rd, n);
            col += vec3(1.0) * pow(max(dot(ref, l), 0.0), 32.0) * 0.6;
        } else {
            // Infinite Mathematical Void gradient
...
export const MathArtLatticeHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#010002] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(1, 0, 2, 0.9) 100%)' }} />
