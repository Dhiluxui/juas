...
const shaderData = {
  ...
  fragment: `
      ...
      float map(vec3 p, out float trap) {
          ...
          float power = 8.0 + sin(uTime * 0.2) * 1.5;
          trap = 100.0; 
      ...
      void main() {
          ...
          vec3 ro = vec3(sin(uTime * 0.1) * 2.5, 0.5 + sin(uTime * 0.15) * 0.5, cos(uTime * 0.1) * 2.5);
          ...
          for (int i = 0; i < 80; i++) {
              ...
              if (d < 0.001) {
                  vec3 baseColor = mix(vec3(0.1, 0.5, 1.0), vec3(1.0, 0.8, 0.2), clamp(trap * 1.5, 0.0, 1.0));
                  
                  vec2 e = vec2(0.001, 0.0);
                  float dummyTrap;
                  vec3 n = normalize(vec3(
                      map(p + e.xyy, dummyTrap) - map(p - e.xyy, dummyTrap),
                      map(p + e.yxy, dummyTrap) - map(p - e.yxy, dummyTrap),
                      map(p + e.yyx, dummyTrap) - map(p - e.yyx, dummyTrap)
                  ));
                  
                  float diff = max(dot(n, normalize(vec3(1.0, 1.0, -1.0))), 0.0);
                  float amb = 0.2;
                  
                  col = baseColor * (diff + amb) * exp(-0.1 * t);
                  break;
              }
          ...
          col += vec3(0.0, 0.2, 0.5) * (0.05 / (trap + 0.01)) * exp(-0.5 * t);
...
export const MandelbulbFractalHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#000205] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(0, 2, 5, 0.8) 100%)' }} />
