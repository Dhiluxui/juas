...
const shaderData = {
  ...
  fragment: `
    ...
    // Signed Distance Function for the Nexus Core
    float map(vec3 p) {
        p.xz *= rot(uTime * 0.2);
        p.xy *= rot(uTime * 0.15);
        
        // Infinite folding structure
        for(int i = 0; i < 4; i++) {
            p = abs(p) - 0.5;
            p.xz *= rot(0.5);
            p.yz *= rot(0.3);
    ...
    void main() {
        ...
        vec3 rd = normalize(vec3(uv + mouse * 0.2, 1.5)); // Ray direction
        ...
        // Coloring based on distance and normal-ish approximation
        vec3 col = vec3(0.0);
        if(t < 10.0) {
            float glow = 1.0 / (1.0 + t * t * 0.5);
            col = vec3(0.1, 0.4, 0.6) * glow;
            col += vec3(0.6, 0.2, 0.8) * (1.0 - glow); // Nebula shift
            col *= 1.5;
        }
...
export const EtherealEnergyNexusHero = ({ className = '', children, ...props }: any) => (
