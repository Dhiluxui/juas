...
const shaderData = {
  ...
  fragment: `
    ...
    float map(vec3 p) {
        ...
        // Wrap space into an infinite grid
        p = mod(p, 4.0) - 2.0;
        
        // Fractal folding loop
        for(int i = 0; i < 6; i++) {
            // Box fold
            p = 2.0 * clamp(p, -vec3(1.2), vec3(1.2)) - p;
            
            // Sphere fold (inverts space)
            float r2 = dot(p,p);
            float k = max(1.2 / r2, 1.0);
            p *= k;
            scale *= k;
            
            // Offset and rotate for complexity
            p += vec3(0.5, 0.5, 0.5);
            p.xy *= rot(0.5);
            p.yz *= rot(uTime * 0.05 + 0.5);
        }
        
        // Distance to the complex folded structure
    ...
    void main() {
        ...
        // Glow accumulator for the deep cracks
        float glow = 0.0;
        ...
        for(int i = 0; i < 100; i++) {
            ...
            // Accumulate subsurface energy (magma/gold in the cracks)
            glow += 0.002 / (0.01 + abs(d)) * exp(-dTotal * 0.5);
            
            if(d < 0.002 || dTotal > 15.0) break;
            
            // KIFS fractals have severe distance distortions, require small steps
            dTotal += d * 0.6;
        }
        ...
        if (dTotal < 15.0) {
            ...
            vec3 baseCol = vec3(0.01);
            
            // Razor sharp specular highlights to make it look like polished glass/stone
            vec3 l = normalize(vec3(1.0, 2.0, -3.0)); // Main light
            float diff = max(dot(n, l), 0.0);
            
            vec3 ref = reflect(rd, n);
            float spec = pow(max(dot(ref, l), 0.0), 128.0); // Extremely tight highlight
            
            // Secondary colored light (ambient from the magma)
            vec3 l2 = normalize(vec3(-1.0, -1.0, 1.0));
            float spec2 = pow(max(dot(ref, l2), 0.0), 32.0);
            
            col = baseCol * diff;
            col += vec3(1.0, 0.9, 0.8) * spec * 2.0; // White reflections
            col += vec3(1.0, 0.3, 0.0) * spec2 * 1.5; // Orange reflections
            
            // Fresnel rim highlighting
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 4.0);
            col += vec3(1.0, 0.4, 0.0) * fresnel * 0.5;
        }
        
        // Add the volumetric magma glow emitting from the deep fractal crevices
        col += vec3(1.0, 0.2, 0.0) * glow * 1.5; // Intense red/orange heat
        
        // Fade to pitch black deep in the cavern
        col = mix(col, vec3(0.0), smoothstep(5.0, 15.0, dTotal));
...
export const ObsidianLatticeHero = ({ className = '', children, ...props }: any) => (
