...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
        ...
        for (int i = 0; i < 6; i++) {
            p = abs(p) / dot(p, p) - 0.7; 
            p *= rot(uTime * 0.2 + float(i) * 0.5);
            p *= 1.2;
        }

        // Coloring based on fold distance
        float intensity = 0.05 / length(p);
        
        vec3 color = vec3(0.1, 0.2, 0.5) * intensity;
        color += vec3(0.3, 0.6, 0.9) * intensity * 0.5;
        color += vec3(0.9, 0.9, 1.0) * pow(intensity, 2.0);
...
export const RecursiveCausticHero = ({ className = '', children, ...props }: any) => (
