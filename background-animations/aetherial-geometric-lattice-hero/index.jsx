...
const shaderData = {
  ...
  fragment: `
            ...
            float map(vec3 p) {
                ...
                return sdTorus(p, vec2(0.8, 0.2));
            }

            void main() {
                vec2 uv = (gl_FragCoord.xy - 0.5 * uResolution.xy) / min(uResolution.y, uResolution.x);
                vec3 ro = vec3(0.0, 0.0, -3.0); // Camera
                vec3 rd = normalize(vec3(uv, 1.2)); // Ray dir
                
                // Add slight parallax from mouse
                vec2 m = (uMouse / uResolution - 0.5) * 0.5;
                rd.xy *= rot(m.x);
                ...
                col += glow * vec3(0.1, 0.6, 0.9) * 0.05;
                col += glow * glow * vec3(0.8, 0.2, 0.5) * 0.01;
...
export const AetherialGeometricLatticeHero = ({ className = '', children, ...props }: any) => (
