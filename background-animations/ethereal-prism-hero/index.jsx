...
const shaderData = {
  ...
  fragment: `
    ...
    // Exact Distance to Octahedron (Prism shape)
    ...
    float map(vec3 p) {
        ...
        // The main glass prism
        float d1 = sdOctahedron(p, 1.5);
        
        // Suspended glowing energy core inside the prism
        float d2 = length(p) - 0.4 + sin(uTime * 4.0) * 0.05;
    ...
    // Volumetric Ethereal Nebula Background
    float nebula(vec3 dir) {
        float n = sin(dir.x * 12.0 + uTime) * cos(dir.y * 12.0 - uTime) * sin(dir.z * 12.0);
    ...
    void main() {
        ...
        for(int i = 0; i < 90; i++) {
            ...
            // Core glow accumulation
            float core = length(p) - 0.4;
            glow += 0.006 / (0.01 + abs(core));
            
            if(d < 0.001 || dTotal > 12.0) break;
            dTotal += d * 0.8;
        }
        
        vec3 col = vec3(0.01, 0.015, 0.03); // Void space
        ...
        if (dTotal < 12.0) {
            ...
            // Fake Chromatic Dispersion (Refracting RGB separately)
            // Simulates thick, highly refractive, expensive diamond glass
            vec3 refrR = refract(i, n, 0.65);
            vec3 refrG = refract(i, n, 0.67);
            vec3 refrB = refract(i, n, 0.69);
            
            // Sample background nebula using the refracted rays
            float envR = nebula(refrR);
            float envG = nebula(refrG);
            float envB = nebula(refrB);
            
            // Intense Specular reflection from an unseen light source
            ...
            // Composite the glass material
            col = vec3(envR, envG, envB) * 1.5; // Iridescent refracted background
            col += vec3(1.0) * spec * 3.0;      // Hard diamond specular
            col += vec3(0.6, 0.8, 1.0) * fresnel * 2.5; // Glass edges
            
            // Add the glowing core suspended inside the glass
            col += vec3(1.0, 0.1, 0.5) * glow * 0.4;
        } else {
            // Missed the object: Render the background ethereal nebula
            float env = nebula(rd);
            col += mix(vec3(0.1, 0.0, 0.3), vec3(0.0, 0.6, 1.0), env) * 0.4;
        }
...
export const EtherealPrismHero = ({ className = '', children, ...props }: any) => (
