...
const shaderData = {
  ...
  fragment: `
      ...
      // Better 3D noise for soft volumetric clouds
      ...
      void main() {
          ...
          for(int i = 0; i < 50; i++) {
              ...
              // Move and rotate space
              p.xy *= rot(uTime * 0.1);
              p.z += uTime * 0.2;
              
              // Raymarch volumetric filaments
              float n = ridgedFBM(p * 1.5);
              
              // Isolate the sharpest ridges
              float mask = smoothstep(0.75, 1.0, n);
              
              if(mask > 0.0) {
                  // Bioluminescent Dark Matter Colors
                  vec3 c1 = vec3(1.0, 0.1, 0.3); // Crimson
                  vec3 c2 = vec3(0.2, 0.0, 1.0); // Deep Violet
                  vec3 c3 = vec3(1.0, 0.5, 0.0); // Orange highlight
                  
                  vec3 cloudCol = mix(c2, c1, sin(p.y * 3.0 + uTime) * 0.5 + 0.5);
                  cloudCol = mix(cloudCol, c3, mask * 0.8); // Core is orange
                  
                  col += cloudCol * mask * 0.08;
                  density += mask * 0.1;
              }
              
              // Fast marching since it's volumetric
              t += 0.1;
              if(density > 1.5) break;
          }
          
          col += vec3(0.02, 0.0, 0.05) * (1.0 - length(uv)); // Abyss background
...
export const NebulaFractureV2Hero = ({ className = '', children, ...props }: any) => (
