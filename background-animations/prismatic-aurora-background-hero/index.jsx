...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
        ...
        float t = uTime * 0.5;
        float w1 = sin(r * 5.0 - t + a * 2.0);
        float w2 = cos(r * 3.0 + t * 0.7 + a * 3.0);
        float w3 = sin(r * 8.0 - t * 1.2 + sin(a) * 4.0);
        
        float waves = (w1 + w2 + w3) / 3.0;
        
        vec3 col = vec3(0.5) + 0.5 * cos(3.0 + waves * 4.0 + vec3(0.0, 2.0, 4.0));
        
        float vignette = 1.0 - smoothstep(0.2, 1.5, r);
        col *= vignette;
        
        col = mix(col, vec3(0.1, 0.05, 0.2), 0.2);
...
export const PrismaticAuroraBackgroundHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-black overflow-hidden font-sans ${className}`}>
