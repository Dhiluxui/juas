...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          vec2 p = uv * 3.0;
          p += vec2(sin(uTime * 0.4 + p.y * 0.5), cos(uTime * 0.3 + p.x * 0.5));

          float v1 = smoothVoronoi(p);
          float v2 = smoothVoronoi(p * 2.0 - uTime * 0.2);

          float diff = abs(v1 - v2 * 0.5);

          float membrane = smoothstep(0.15, 0.1, diff) * smoothstep(0.05, 0.1, diff);
          float nucleus = smoothstep(0.05, 0.0, diff);

          vec3 col = vec3(0.0, 0.05, 0.1);                       
          col = mix(col, vec3(0.0, 1.0, 0.4), membrane * 1.5);   
          col = mix(col, vec3(0.0, 0.6, 1.0), nucleus);          
...
export const AlienTissueHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#00050a] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(0, 5, 10, 0.8) 100%)' }} />
