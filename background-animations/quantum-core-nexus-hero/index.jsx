  ...
  fragment: `
    ...
    void main() {
        ...
        for(int i = 0; i < 64; i++) {
            p = ro + rd * dTotal;
            float d = map(p);
            glow += 0.05 / (0.1 + d * d * 10.0);
            dTotal += d * 0.5;
            if(d < 0.001 || dTotal > 10.0) break;
        }
        
        vec3 col = vec3(0.1, 0.2, 0.4); 
        col += glow * vec3(0.5, 0.8, 1.0) * 0.1; 
        col += vec3(0.2, 0.1, 0.3) * (dTotal * 0.1); 
...
export const QuantumCoreNexusHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020205] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(2, 2, 5, 0.8) 100%)' }} />
