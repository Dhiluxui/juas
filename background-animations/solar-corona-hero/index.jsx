...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          for(int j = 1; j <= 6; j++) {
              float i = float(j);
              flare += sin(a * pow(2.0, i) + uTime * i * 1.5) / pow(2.0, i);
          }
          ...
          float dist = abs(r - baseRadius - flare * 0.08);
          
          float glow = 0.005 / (dist + 0.001);
          
          vec3 col = vec3(1.0, 0.4, 0.05) * glow;
          
          col += vec3(0.2, 0.5, 1.0) * (0.01 / (abs(r - baseRadius) + 0.001));
          
          col *= smoothstep(baseRadius - 0.05, baseRadius + 0.05, r);
          
          float stars = fract(sin(dot(uv, vec2(12.9898, 78.233))) * 43758.5453);
          stars = step(0.995, stars) * (sin(uTime * 3.0 + r * 10.0) * 0.5 + 0.5);
          col += vec3(stars);
...
export const SolarCoronaHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#010308] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(1, 3, 8, 0.8) 100%)' }} />
