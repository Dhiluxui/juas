...
const shaderData = {
  ...
  fragment: `
    ...
    // Map a thin, undulating 3D sheet of ethereal fabric
    float map(vec3 p) {
        float t = uTime * 0.4;
        
        // Create an organic heightmap using FBM
        float h = fbm(p.xz * 0.8 - vec2(t)) * 1.5;
        
        // Add sweeping, low-frequency geometric folds (like silk in the wind)
        h += sin(p.x * 2.0 + t) * 0.5;
        h += cos(p.z * 1.5 - t * 0.5) * 0.3;
        
        // Constrain the distance field to a very thin sheet along the Y axis
        // We use absolute value to create a physical front and back face.
        return abs(p.y - h) - 0.02; // 0.02 is the thickness of the fabric
    }
    ...
    // Abstract background environment to refract through the fabric
    ...
    void main() {
        ...
        // Camera flying directly over the undulating fabric
        vec3 ro = vec3(0.0, 2.0, -3.0 + uTime); 
        ...
        // Raymarch the thin fabric
        for(int i = 0; i < 100; i++) {
            p = ro + rd * dTotal;
            float d = map(p);
            if(d < 0.005 || dTotal > 15.0) break;
            
            // Extremely conservative step size because thin sheets are notoriously difficult to raymarch without clipping
            dTotal += d * 0.6; 
        }
        
        vec3 col = vec3(0.01, 0.015, 0.02); // Void
        
        if (dTotal < 15.0) {
            vec3 n = getNormal(p);
            vec3 i = rd;
            
            // Extreme Refraction simulating clear, iridescent liquid or glass silk
            vec3 refr = refract(i, n, 0.85);
            float env = nebula(refr);
            
            // High gloss specular highlights
            vec3 l = normalize(vec3(1.0, 3.0, -1.0));
            float spec = pow(max(dot(reflect(rd, n), l), 0.0), 64.0);
            
            // Iridescent Flow Colors mapping to the physical normal of the folds
            vec3 flowColor = mix(vec3(0.0, 0.8, 1.0), vec3(1.0, 0.2, 0.8), n.y * 0.5 + 0.5);
            
            // Brilliant glowing edges
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 3.0);
            
            // Composite the ethereal fabric
            col = flowColor * env * 2.5; // Iridescent background distortion
            col += vec3(1.0) * spec * 2.0; // Blinding glass shine
            col += vec3(1.0, 1.0, 1.0) * fresnel * 1.5; // Pearlescent edges
        } else {
            // Missed the fabric: render the background nebula directly
            float env = nebula(rd);
            col += mix(vec3(0.0, 0.1, 0.2), vec3(0.8, 0.2, 0.6), env) * 0.3;
        }
        
        // Depth of field fog
        col = mix(col, vec3(0.01, 0.015, 0.02), smoothstep(8.0, 15.0, dTotal));
...
export const EtherealFlowHero = ({ className = '', children, ...props }: any) => (
