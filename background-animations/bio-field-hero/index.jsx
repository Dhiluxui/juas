...
const shaderData = {
  ...
  fragment: `
    ...
    // Smooth min for blending organic cellular structures
    float smin(float a, float b, float k) {
        float h = clamp(0.5 + 0.5 * (b - a) / k, 0.0, 1.0);
        return mix(b, a, h) - k * h * (1.0 - h);
    }

    // Map the 3D Bio-Field Environment
    float map(vec3 p) {
        float t = uTime * 0.5;
        
        // Flowing fluid background membrane (Tunnel walls)
        float d = -p.y + 2.0 + sin(p.x * 2.0 + t) * 0.5 + cos(p.z * 1.5 - t) * 0.5;
        float floorD = p.y + 2.0 + cos(p.x * 2.0 - t) * 0.5 + sin(p.z * 1.5 + t) * 0.5;
        d = smin(d, floorD, 2.0);
        
        // Infinite field of floating bio-cells / viruses
        vec3 q = p;
        float spacing = 3.5;
        vec3 id = floor(q / spacing);
        q = mod(q, spacing) - spacing * 0.5;
        
        // The cells drift organically through the fluid
        q.x += sin(t + id.y * 2.0);
        q.y += cos(t * 0.8 + id.z);
        q.z += sin(t * 1.2 + id.x);
        
        // Cellular bodies (pulsating spheres)
        float pulse = sin(uTime * 4.0 + id.x * 5.0) * 0.1;
        float cell = length(q) - 0.5 - pulse;
        
        // Add high-frequency organic bumps to the cell surface
        cell += sin(q.x * 12.0 + t) * sin(q.y * 12.0) * sin(q.z * 12.0) * 0.04;
        
        // Smoothly blend the cells with the outer membranes when they collide
        return smin(d, cell, 1.2);
    }
    ...
    void main() {
        ...
        // Camera smoothly flows forward through the bloodstream
        ...
        // Gentle organic camera drift and tilt
        rd.xy *= rot(sin(uTime * 0.3) * 0.2);
        ...
        for(int i = 0; i < 80; i++) {
            ...
            // Subsurface bioluminescent scattering inside the fluid
            bioGlow += 0.005 / (0.01 + abs(d)) * exp(-dTotal * 0.15);
            
            if(d < 0.001 || dTotal > 20.0) break;
            
            // Dampen step size because of smooth mins and displacement
            dTotal += d * 0.7; 
        }
        
        vec3 col = vec3(0.01, 0.03, 0.02); // Murky deep green/cyan plasma
        ...
        if (dTotal < 20.0) {
            ...
            // Wet, specular organic membrane material
            col = vec3(0.02, 0.1, 0.05) * diff;
            
            // Glossy liquid reflections
            vec3 ref = reflect(rd, n);
            float spec = pow(max(dot(ref, l), 0.0), 16.0);
            col += vec3(0.5, 1.0, 0.8) * spec * 0.8;
            
            // Bio-electric rim lighting on the cell edges
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 4.0);
            vec3 rimColor = mix(vec3(0.0, 1.0, 0.5), vec3(0.0, 0.5, 1.0), sin(uTime + p.z) * 0.5 + 0.5);
            col += rimColor * fresnel * 1.5;
        }
        
        // Intense volumetric internal bio-glow from the cells
        col += vec3(0.0, 1.0, 0.6) * bioGlow * 0.6;
        
        // Thick plasma fog
        col = mix(col, vec3(0.01, 0.03, 0.02), smoothstep(10.0, 20.0, dTotal));
...
export const BioFieldHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#010302] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(1, 3, 2, 0.95) 100%)' }} />
