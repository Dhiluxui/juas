...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
        ...
        uv += normalize(uv - mouse) * (0.2 / (dMouse + 0.2));

        vec3 finalCol = vec3(0.0);
        
        for(int j = 0; j < 8; j++) {
            float i = float(j);
            float t = uTime * 0.5 + i * 0.4;
            
            vec2 pos = vec2(cos(t), sin(t)) * 0.4;
            
            pos.x += sin(uTime * 0.8 + i) * 0.1;
            pos.y += cos(uTime * 0.5 + i) * 0.1;
            
            float d = length(uv - pos);
            
            float glow = 0.015 / (d * d * 10.0 + 0.01);
            
            vec3 col = palette(i / TEND_COUNT + uTime * 0.1);
            finalCol += col * glow;
        }
        
        float core = 0.005 / length(uv);
        finalCol += core * vec3(0.8, 0.9, 1.0);
...
export const BioluminescentQuantumCoreHero = ({ className = '', children, ...props }: any) => (
