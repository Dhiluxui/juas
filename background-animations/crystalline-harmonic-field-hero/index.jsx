...
const shaderData = {
  ...
  fragment: `
    ...
    // Complex rotation matrix for harmonic bending
    mat2 rot(float a) {
    ...
    void main() {
        ...
        float t = iTime * 0.2;
        uv *= 1.5 + 0.5 * sin(t * 0.5);
        uv *= rot(t * 0.2);

        vec3 col = vec3(0.0);
        
        // Harmonic accumulation: Creating structural geometric interference
        // Instead of noise, we use multiple layers of structured sine waves
        float dist = 0.0;
        for(float i = 1.0; i < 6.0; i++) {
            uv.x += 0.3 / i * sin(i * uv.y + t + cos(uv.x * 2.0));
            uv.y += 0.3 / i * cos(i * uv.x + t + sin(uv.y * 2.0));
            dist += length(uv);
        }

        // Color mapping: Using structural highlights for a "glass" effect
        float val = sin(dist * 0.5 + t);
        col += vec3(0.1, 0.2, 0.4) * (0.5 + 0.5 * val);
        col += vec3(0.3, 0.6, 0.9) * (0.5 + 0.5 * sin(dist * 0.2));
        
        // Add crystalline sharp edges via power function
        col = pow(col, vec3(1.2)) * 1.5;
...
export const CrystallineHarmonicFieldHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#08080a] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(8, 8, 10, 0.8) 100%)' }} />
