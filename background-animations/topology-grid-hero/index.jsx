...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
      vec2 p = (gl_FragCoord.xy * 2.0 - uResolution.xy) / uResolution.y * 7.5;
      
      float a = uTime * 0.2;
      mat2 rot = mat2(cos(a), -sin(a), sin(a), cos(a));
      p = rot * p;
      
      float val = sin(p.x + p.y) + sin(p.x - p.y);
      float edge = step(0.8, abs(sin(val * 3.0)));
      
      vec3 col = vec3(0.05, 0.1, 0.2);
      col = mix(col, vec3(0.3, 0.8, 1.0), edge * 0.5);
      col += vec3(0.2, 0.5, 0.9) * (1.0 - length(p) / 10.0);
...
export const TopologyGridHero = ({ className = '', children, ...props }: any) => (
