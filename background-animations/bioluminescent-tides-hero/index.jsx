...
const shaderData = {
  ...
  fragment: `
    ...
    // 2D Hash and Noise for Oceanic Waves
    ...
    float map(vec3 p) {
        ...
        // Flowing ocean waves (Simulating Gerstner waves via domain-warped FBM)
        vec2 uv = p.xz;
        uv.x += t * 0.5; // The rolling tides pushing forward
        
        // Domain warp to create wind-swept turbulent peaks
        uv.y += sin(uv.x * 0.5 + t) * 0.5;
        
        // Evaluate the wave height
        float waves = fbm(uv * 1.5 - vec2(t * 1.5, 0.0));
        
        // Square the heightmap to sharpen the wave peaks and flatten the troughs
        waves = pow(waves, 2.0) * 1.5;
        
        // Subtract the wave height from the sea level
        return (d - waves) * 0.5; // Step conservatively due to high frequency surface
    }
    ...
    void main() {
        ...
        // Camera hovering very low over the ocean, facing the incoming tides
        ...
        vec3 col = vec3(0.01, 0.02, 0.04); // Deep dark night sky
        ...
        if (dTotal < 30.0) {
            ...
            // Base dark water material
            col = vec3(0.0, 0.05, 0.1) * diff;
            
            // Moonlight glinting off the water surface
            vec3 ref = reflect(rd, n);
            float spec = pow(max(dot(ref, l), 0.0), 32.0);
            col += vec3(0.5, 0.8, 1.0) * spec * 0.6;
            
            // BIOLUMINESCENCE LOGIC:
            // The water glows intensely cyan/blue only at the very peaks of the crashing waves.
            // We calculate this using the physical Y height of the geometry, mixed with the normal direction.
            float bioMask = smoothstep(-0.8, -0.2, p.y) * max(0.0, dot(n, normalize(vec3(-1.0, 0.5, 0.0))));
            
            // Dynamic alien bio-color shifting over time and space
            vec3 bioColor = mix(vec3(0.0, 1.0, 0.8), vec3(0.0, 0.5, 1.0), sin(p.x * 2.0 - uTime) * 0.5 + 0.5);
            
            // Add the glowing crashing tide effect
            col += bioColor * bioMask * 3.0; 
            
            // Soft atmospheric sea fog in the distance
            col = mix(col, vec3(0.01, 0.02, 0.04), smoothstep(10.0, 30.0, dTotal));
        } else {
            // Render a starry night sky if the ray misses the ocean
            float stars = pow(fract(sin(dot(rd.xy, vec2(12.9898, 78.233))) * 43758.5453), 150.0);
            col += vec3(1.0, 1.0, 0.9) * stars * 0.8;
        }
...
export const BioluminescentTidesHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#010204] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(1, 2, 4, 0.9) 100%)' }} />
