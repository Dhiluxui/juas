...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          uv += vec2(sin(t * 0.5 + uv.y * 2.0), cos(t * 0.6 + uv.x * 1.5)) * 0.2;
          ...
          float networkEdges = 0.015 / (edgeDist + 0.005);
          float nodes = 0.02 / (minDist1 + 0.01);
          
          vec3 edgeColor = mix(vec3(0.0, 0.3, 0.8), vec3(0.0, 0.9, 1.0), sin(minDist1 * 10.0 - t * 2.0) * 0.5 + 0.5);
          vec3 nodeColor = vec3(0.8, 0.9, 1.0);
          
          vec3 finalColor = (edgeColor * networkEdges) + (nodeColor * nodes * 0.5);
          
          finalColor += vec3(0.01, 0.01, 0.03);
          finalColor *= smoothstep(2.5, 0.5, length(uv));
...
export const NeuralWebHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020408] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(2, 4, 8, 0.8) 100%)' }} />
