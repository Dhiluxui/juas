...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          float sun = smoothstep(0.4, 0.38, sunDist);
          
          float stripes = step(0.15, fract(p.y * 15.0 - uTime * 0.5));
          sun *= mix(1.0, stripes, step(p.y, 0.3));
          
          vec3 sunCol = mix(vec3(1.0, 0.0, 0.6), vec3(1.0, 0.9, 0.0), p.y + 0.2);
          col += sunCol * sun;

          if (p.y < 0.0) {
              vec2 gridUv = vec2(p.x / abs(p.y), 1.0 / abs(p.y) + uTime * 2.0);
              vec2 grid = fract(gridUv * 1.5);
              
              float line = smoothstep(0.0, 0.05, grid.x) * smoothstep(1.0, 0.95, grid.x);
              line *= smoothstep(0.0, 0.05, grid.y) * smoothstep(1.0, 0.95, grid.y);
              
              float gridAlpha = 1.0 - line;
              gridAlpha *= smoothstep(0.0, 0.6, abs(p.y));
              
              col += vec3(0.0, 1.0, 1.0) * gridAlpha;
          }

          col += vec3(1.0, 0.0, 0.5) * (0.015 / (abs(p.y) + 0.005));
...
export const SynthwaveGridHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020005] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(2, 0, 5, 0.8) 100%)' }} />
