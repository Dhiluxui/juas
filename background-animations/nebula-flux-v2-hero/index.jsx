...
const shaderData = {
  ...
  fragment: `
    ...
    // 3D Noise for Volumetric Plasma
    float hash(vec3 p) {
    ...
    void main() {
        ...
        // Forward flight into the nebula
        ro.z += t * 3.0;
        
        // Gentle cinematic camera roll
        ro.xy *= rot(t * 0.2); 
        rd.xy *= rot(t * 0.2);
        ...
        for(int i = 0; i < 60; i++) {
            ...
            // Warp spacetime to create twisting flux tubes
            vec3 q = p;
            q.x += sin(p.z * 0.5 + t);
            q.y += cos(p.z * 0.4 + t * 0.8);
            
            // Evaluate density using Fractal Brownian Motion
            float density = fbm(q * 0.8 - vec3(0.0, 0.0, t));
            
            // Hollow out an immense tunnel for the camera to fly through
            float tunnel = length(q.xy) - 1.5;
            density *= smoothstep(-0.5, 1.0, tunnel); // Cosmic dust concentrates at the edges
            
            if (density > 0.01) {
                // Intense Galactic Plasma Palette
                vec3 nebulaCol = mix(vec3(0.1, 0.0, 0.5), vec3(1.0, 0.2, 0.4), density * 2.0); // Deep purple to hot magenta
                nebulaCol = mix(nebulaCol, vec3(0.0, 1.0, 0.8), sin(p.z * 0.5) * 0.5 + 0.5);   // Swirls of cyan
                
                // Light attenuation (darker further away)
                float att = exp(-dTotal * 0.1);
                
                // Accumulate color volumetrically
                col += nebulaCol * density * att * 0.08;
            }
            
            // Dynamic step size (faster in empty space)
            dTotal += max(0.1, 0.05 + density * 0.2);
            if(dTotal > 15.0) break;
        }
        
        // Add distant shimmering stars
        float stars = pow(hash(rd * 100.0), 100.0);
        col += vec3(1.0) * stars * 0.5 * exp(-dTotal * 0.05);

        // Cinematic Post-Processing
        col = pow(col, vec3(0.85)); // Gamma curve
        col *= 1.0 - dot(uv, uv) * 0.5; // Heavy vignette
...
export const NebulaFluxV2Hero = ({ className = '', children, ...props }: any) => (
