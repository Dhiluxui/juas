...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          vec2 p = abs(localUv);
          float dist = max(p.x, p.x * 0.5 + p.y * 0.8660254); 
          
          float border = smoothstep(0.45, 0.5, dist) * smoothstep(0.55, 0.5, dist);
          
          float rand = fract(sin(dot(cellId, vec2(12.9898, 78.233))) * 43758.5453);
          
          float pulse = sin(uTime * 2.0 + rand * 6.28) * 0.5 + 0.5;
          
          vec3 col = vec3(0.0, 0.8, 1.0) * border * pulse * 1.5;
          
          col += vec3(0.0, 0.15, 0.3) * smoothstep(0.5, 0.0, dist) * pulse;
...
export const HexagonalLatticeHero = ({ className = '', children, ...props }: any) => (
