...
const shaderData = {
  ...
  fragment: `
    ...
    // Smooth Minimum for organic, membrane-like blending
    float smin(float a, float b, float k) {
        float h = clamp(0.5 + 0.5 * (b - a) / k, 0.0, 1.0);
        return mix(b, a, h) - k * h * (1.0 - h);
    }

    // Map the deep-sea void geometry
    float map(vec3 p) {
        float t = uTime * 0.3;
        
        // Infinite space repetition
        vec3 q = p;
        q.x = mod(q.x + 3.0, 6.0) - 3.0;
        q.y = mod(q.y + 3.0, 6.0) - 3.0;
        q.z = mod(q.z + 3.0, 6.0) - 3.0;
        
        // Organic undulating tendrils
        float dx = length(q.xy - vec2(sin(p.z + t), cos(p.z * 0.8 + t))) - 0.15;
        float dy = length(q.xz - vec2(cos(p.y * 1.2 - t), sin(p.y + t))) - 0.15;
        float dz = length(q.yz - vec2(sin(p.x * 0.9 + t), cos(p.x * 1.1 - t))) - 0.15;
        
        // Blend tendrils smoothly where they intersect
        float d = smin(dx, smin(dy, dz, 0.8), 0.8);
        
        // Add pulsing bioluminescent pods at intersections
        float pods = length(q) - 0.4 - sin(uTime * 3.0 + p.x) * 0.1;
        d = smin(d, pods, 1.2);
    ...
    void main() {
        ...
        // Fly through the void over time
        ...
        for(int i = 0; i < 90; i++) {
            ...
            // Subsurface bioluminescent scattering (glow inside the membrane)
            glow += 0.006 / (0.01 + abs(d)) * exp(-dTotal * 0.15);
            
            dTotal += abs(d) * 0.7; // Dampen step size for smooth mins
            if(dTotal > 20.0) break;
        }
        
        // Murky deep-ocean/abyssal void
        vec3 col = vec3(0.01, 0.03, 0.02);
        
        // Bioluminescent Cyan/Green palette that shifts over space and time
        vec3 bioCol = mix(vec3(0.0, 1.0, 0.6), vec3(0.0, 0.4, 1.0), sin(uTime + dTotal * 0.5) * 0.5 + 0.5);
        
        col += bioCol * glow * 1.2;
        col += vec3(0.8, 1.0, 0.9) * pow(glow * 0.25, 3.0); // Bright white-hot core
...
export const BioluminescentVoidHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#010302] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(1, 3, 2, 0.9) 100%)' }} />
