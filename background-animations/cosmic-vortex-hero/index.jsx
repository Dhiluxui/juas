...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          float spiral = a - log(r + 0.001) * 3.0;
          
          for (int j = 1; j <= 3; j++) {
              float i = float(j);
              float offset = i * 2.094; 
              
              float arm = sin(spiral * 4.0 + offset - t * (2.0 + i));
              
              float envelope = smoothstep(0.0, 0.3, r) * smoothstep(1.5, 0.1, r);
              arm = smoothstep(0.5, 1.0, arm);
              
              float glow = 0.05 / (abs(arm - 1.0) + 0.05) * envelope;
              vec3 armColor = 0.5 + 0.5 * cos(t + i + vec3(0.0, 1.2, 2.4));
              
              finalColor += armColor * glow;
          }

          float horizon = smoothstep(0.15, 0.2, r);
          finalColor *= horizon;
          finalColor += vec3(0.6, 0.2, 0.8) * (0.01 / (r + 0.01));
...
export const CosmicVortexHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#030105] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(3, 1, 5, 0.8) 100%)' }} />
