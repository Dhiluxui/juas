...
const shaderData = {
  ...
  fragment: `
    ...
    vec2 distort(vec2 p, float t, vec2 m) {
        float d = length(p - m);
    ...
    void main() {
        ...
        for(int j = 1; j < 4; j++) {
            float i = float(j);
            q = distort(q, uTime * 0.2 * i, mouse);
        }

        float r = length(q + 0.01 * sin(uTime));
        float g = length(q + 0.02 * cos(uTime));
        float b = length(q + 0.03 * sin(uTime * 0.5));

        vec3 col = vec3(0.0);
        col += palette(r * 0.5 + uTime * 0.1) * 0.4;
        col += palette(g * 0.5 + uTime * 0.2) * 0.3;
        col += palette(b * 0.5 + uTime * 0.3) * 0.3;

        float edge = 1.0 - smoothstep(0.3, 0.8, length(q));
        col *= edge * 2.0;
...
export const LiquidChromaticPrismHero = ({ className = '', children, ...props }: any) => (
