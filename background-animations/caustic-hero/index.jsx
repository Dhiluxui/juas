...
const shaderData = {
  ...
  fragment: `
    ...
    void main() {
      vec2 uv = (gl_FragCoord.xy * 2.0 - uResolution.xy) / uResolution.y * 2.0;
      
      float t = uTime * 0.5;
      vec2 p = uv;
      for(int i = 0; i < 3; i++) {
          p.x += 0.3 * sin(p.y + t) + 0.3 * cos(p.x + t);
          p.y += 0.3 * cos(p.x + t) + 0.3 * sin(p.y + t);
      }
      
      float r = sin(p.x + p.y) * 0.5 + 0.5;
      float g = cos(p.x - p.y) * 0.5 + 0.5;
      float b = sin(p.x * 2.0) * 0.5 + 0.5;
      
      vec3 col = vec3(r, g, b) * 0.5;
      col += vec3(0.2, 0.4, 0.6) * (1.0 - length(uv) * 0.5);
...
export const CausticHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020202] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(2, 2, 2, 0.8) 100%)' }} />
