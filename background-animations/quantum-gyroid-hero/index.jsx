...
const shaderData = {
  ...
  fragment: `
    ...
    float sdGyroid(vec3 p) {
      p *= 1.5;
      return abs(dot(sin(p), cos(p.zxy))) * 0.25 - 0.04;
    }
    ...
    void main() {
      ...
      for (int i = 0; i < 70; i++) {
        ...
        glow += 0.005 / (0.02 + abs(d)); 
        
        if (abs(d) < 0.002 || t > 15.0) break;
      ...
      if (t < 15.0) {
        ...
        vec3 lightColor1 = vec3(1.0, 0.3, 0.0);
        vec3 lightColor2 = vec3(0.3, 0.1, 1.0);
        vec3 baseMaterial = mix(lightColor1, lightColor2, sin(p.z * 1.5 - uTime) * 0.5 + 0.5);
        ...
        col = baseMaterial * diff * 0.4;
        col += vec3(0.8, 0.9, 1.0) * fresnel * 1.5;
      }

      vec3 fogColor = mix(vec3(0.5, 0.1, 0.9), vec3(1.0, 0.6, 0.1), sin(uTime * 0.3) * 0.5 + 0.5);
      col += glow * fogColor * 0.15;

      col = mix(col, bg, smoothstep(3.0, 15.0, t));
...
export const QuantumGyroidHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#030208] overflow-hidden font-sans ${className}`}>
    <div className="absolute inset-0 z-0">
      <ShaderBackground vertexShaderSource={shaderData.vertex} fragmentShaderSource={shaderData.fragment} {...props} />
    </div>
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(3, 2, 8, 0.8) 100%)' }} />
