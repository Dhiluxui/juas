...
const shaderData = {
  ...
  fragment: `
    ...
    // Non-Euclidean rotation matrix
    mat2 rot(float a) { float s=sin(a), c=cos(a); return mat2(c, -s, s, c); }

    // Phase-interference function to create "living" tendrils
    float map(vec3 p) {
        p.z += iTime * 0.2;
        p.xy *= rot(p.z * 0.1);
        
        // Iterative space folding (fractal-like structure without noise)
        for(int i = 0; i < 4; i++) {
            p = abs(p) - 0.5;
            p.xy *= rot(0.6 + iTime * 0.05);
            p.xz *= rot(0.3);
        }
        return length(p.xy) - 0.15;
    }
    ...
    void main() {
        ...
        float dO = 0.0;
        float dist = 0.0;
        // Ray-marching for the interference field
        for(int i = 0; i < 48; i++) {
            vec3 p = ro + rd * dO;
            dist = map(p);
            dO += dist * 0.5;
            if(dO > 8.0 || dist < 0.005) break;
        }
        
        // Color mapping: Using sine-based interference for deep, fluid colors
        vec3 color = vec3(0.1, 0.05, 0.2);
        float glow = 1.0 / (1.0 + dO * dO * 0.2);
        color += vec3(0.2, 0.6, 0.9) * glow;
        color += vec3(0.8, 0.1, 0.4) * (1.0 - smoothstep(0.0, 2.0, dO));
...
export const NeuralFluxHero = ({ className = '', children, ...props }: any) => (
