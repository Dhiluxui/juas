...
const shaderData = {
  ...
  fragment: `
            ...
            // Math: Rotation helper
            mat2 rot(float a) { float s = sin(a), c = cos(a); return mat2(c, -s, s, c); }

            // Math: SDF Segment (The "connection" line)
            ...
            // Math: SDF Sphere (The "node")
            float sdSphere(vec3 p, float s) { return length(p) - s; }

            // Map: Define the neural scene (Geometric structure)
            float map(vec3 p) {
                p.xy *= rot(uTime * 0.1);
                p.yz *= rot(uTime * 0.15);
                ...
                // Create a star-like structure of floating nodes
                for(float i = 0.0; i < 6.0; i++) {
                    float angle = i * 1.047; // ~60 deg segments
                    vec3 nodePos = vec3(cos(angle + uTime*0.2), sin(angle), sin(angle + uTime*0.1)) * 0.8;
                    d = min(d, sdSphere(p - nodePos, 0.05));
                    
                    // Connection lines between nodes
                    vec3 nextPos = vec3(cos(angle + 1.047 + uTime*0.2), sin(angle + 1.047), sin(angle + 1.047 + uTime*0.1)) * 0.8;
                    d = min(d, sdSegment(p, nodePos, nextPos));
                }
                return d;
            }
            ...
            void main() {
                ...
                for(int i = 0; i < 40; i++) {
                    ...
                    float d = map(p);
                    dTotal += d * 0.6;
                    glow += 0.04 / (0.05 + d * d); // Accumulate light glow
                    if(dTotal > 10.0 || d < 0.001) break;
                }

                // Coloring: Tech aesthetic
                vec3 col = vec3(0.05, 0.1, 0.2) * (1.0 - dTotal * 0.2);
                col += glow * vec3(0.2, 0.5, 0.9) * 0.1;
                col += pow(glow, 2.0) * vec3(0.8, 0.9, 1.0) * 0.02;
...
export const NeuralNexusFieldHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020408] overflow-hidden font-sans ${className}`}>
