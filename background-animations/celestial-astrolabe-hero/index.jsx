...
const shaderData = {
  ...
  fragment: `
      ...
      void main() {
          ...
          for(int i = 0; i < 40; i++) {
              vec3 p = ro + rd * t;
              p.xy *= rot(sin(uTime * 0.2) * 0.2);

              vec3 p1 = p;
              p1.xz *= rot(uTime * 0.8);
              p1.yz *= rot(uTime * 0.4);
              float d1 = sdTorus(p1, vec2(1.2, 0.015));

              vec3 p2 = p;
              p2.xy *= rot(uTime * 0.6);
              p2.xz *= rot(-uTime * 0.5);
              float d2 = sdTorus(p2, vec2(0.9, 0.02));

              vec3 p3 = p;
              p3.yz *= rot(-uTime * 0.7);
              p3.xy *= rot(uTime * 0.3);
              float d3 = sdTorus(p3, vec2(0.6, 0.025));

              float d = min(d1, min(d2, d3));

              col += vec3(0.0, 0.8, 1.0) * 0.0004 / (abs(d1) + 0.001); 
              col += vec3(1.0, 0.0, 0.8) * 0.0005 / (abs(d2) + 0.001); 
              col += vec3(1.0, 0.8, 0.0) * 0.0006 / (abs(d3) + 0.001); 
...
export const CelestialAstrolabeHero = ({ className = '', children, ...props }: any) => (
  <div className={`relative w-full h-full bg-[#020104] overflow-hidden font-sans ${className}`}>
    ...
    <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: 'radial-gradient(circle at center, transparent 30%, rgba(2, 1, 4, 0.8) 100%)' }} />
