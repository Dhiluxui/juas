...
const shaderData = {
  ...
  fragment: `
    ...
    // Map a massive 3D Holographic Globe
    float map(vec3 p) {
        // Holographic displacement based on 3D noise
        // This makes the globe look like it's made of digital data/continents
        float n = noise(p * 2.0 + uTime * 0.5) * 0.5;
        
        // Basic sphere
        float globe = length(p) - (2.5 + n);
        
        // Intersect with a grid of thin planes to create a scanline/sliced hologram look
        float slices = abs(fract(p.y * 10.0 - uTime) - 0.5) * 0.1;
        
        return max(globe, -slices); // Cut slices out of the globe
    }
    ...
    void main() {
        ...
        vec3 ro = vec3(0.0, 0.0, -8.0); // Static camera looking at the globe
        ...
        // Spin the whole scene to make the globe rotate
        ro.xz *= rot(uTime * 0.2);
        rd.xz *= rot(uTime * 0.2);
        
        float dTotal = 0.0;
        vec3 p;
        float hologramEnergy = 0.0;
        
        for(int i = 0; i < 90; i++) {
            p = ro + rd * dTotal;
            float d = map(p);
            
            // Volumetrically accumulate light around the hologram
            hologramEnergy += 0.01 / (0.02 + abs(d));
            
            if(d < 0.01 || dTotal > 15.0) break;
            dTotal += d * 0.8;
        }
        ...
        if (dTotal < 15.0) {
            ...
            // HOLOGRAM MATERIAL
            // Mostly transparent, defined entirely by edges (Fresnel)
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 1.5);
            
            // Glitchy color shifts along the Y axis
            vec3 holoColor = mix(vec3(0.0, 1.0, 0.8), vec3(0.5, 0.0, 1.0), sin(p.y * 5.0 + uTime) * 0.5 + 0.5);
            
            col = holoColor * fresnel * 2.0;
            
            // Draw digital data points on the surface using high-frequency noise
            float dataPoints = pow(noise(p * 20.0), 8.0);
            col += vec3(1.0, 1.0, 1.0) * dataPoints * 5.0;
        }
        
        // Add the blurry, volumetric hologram projection light
        col += vec3(0.0, 0.5, 1.0) * hologramEnergy * 0.15;
        
        // Fake chromatic aberration at the edges of the screen
        float distToCenter = length(uv);
        col.r *= 1.0 + distToCenter * 0.2;
        col.b *= 1.0 - distToCenter * 0.2;
        
        // Scanlines over the camera lens
        col *= 1.0 - sin(gl_FragCoord.y * 2.0) * 0.1;
...
export const NextGenHolographicHero = ({ className = '', children, ...props }: any) => (
