...
const shaderData = {
  ...
  fragment: `
    ...
    float fbm(vec2 p) {
        ...
        for(int i = 0; i < 5; i++) { f += a * noise(p); p = r * p * 2.0; a *= 0.5; }
        return f;
    }

    // Infinite sea of displaced liquid metal
    float map(vec3 p) {
        float floorPlane = p.y + 2.0;
        
        // Animate the flow of the liquid
        vec2 flow = p.xz * 0.15;
        flow.x -= uTime * 0.2;
        flow.y -= uTime * 0.1;
        
        // Massive, chaotic waves using FBM
        float waves = fbm(flow) * 4.0;
        
        // Add a secondary layer of high-frequency ripples
        float ripples = fbm(flow * 4.0 + uTime * 0.5) * 0.5;
        
        return floorPlane - waves - ripples;
    }
    ...
    void main() {
        ...
        // Low angle, cinematic camera flying over the sea
        vec3 ro = vec3(0.0, 1.0, uTime * 3.0);
        vec3 rd = normalize(vec3(uv.x, uv.y - 0.1, 1.0)); // Look slightly down
        ...
        // Camera shake/bobbing over the rough waves
        ro.y += sin(uTime * 2.0) * 0.2;
        rd.xy *= rot(sin(uTime * 0.5) * 0.05);
        ...
        for(int i = 0; i < 100; i++) {
            p = ro + rd * dTotal;
            float d = map(p);
            if(d < 0.01 || dTotal > 60.0) break;
            
            // Slow down stepping drastically near the surface to handle the extreme displacement
            dTotal += d * 0.5; 
        }
        
        vec3 col = vec3(0.01, 0.01, 0.02); // Dark studio sky
        
        if (dTotal < 60.0) {
            vec3 n = getNormal(p);
            
            // THICK LIQUID MERCURY MATERIAL
            vec3 baseCol = vec3(0.02, 0.03, 0.05); // Pitch black metal
            
            // Cinematic warm key light coming from the horizon
            vec3 l1 = normalize(vec3(0.5, 0.2, 1.0));
            // Cool fill light from above
            vec3 l2 = normalize(vec3(-0.5, 1.0, -0.5));
            
            float diff1 = max(dot(n, l1), 0.0);
            float diff2 = max(dot(n, l2), 0.0);
            
            // Extremely sharp specular highlights to make it look wet and metallic
            vec3 ref = reflect(rd, n);
            float spec1 = pow(max(dot(ref, l1), 0.0), 128.0);
            float spec2 = pow(max(dot(ref, l2), 0.0), 32.0);
            
            col = baseCol * (diff1 + diff2);
            
            // Warm sunset/neon orange reflections on the wave crests
            col += vec3(1.0, 0.4, 0.0) * spec1 * 3.0;
            // Deep blue sky reflections
            col += vec3(0.0, 0.3, 1.0) * spec2 * 1.5;
            
            // Fresnel rim lighting for depth
            float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 4.0);
            col += vec3(1.0, 0.5, 0.2) * fresnel * 2.0;
            
            // Add distance fog to blend into the horizon
...
export const FluidTopographyHero = ({ className = '', children, ...props }: any) => (
