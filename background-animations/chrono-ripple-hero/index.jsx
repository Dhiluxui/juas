...
const shaderData = {
  ...
  fragment: `
    ...
    const vec3 uColorA = vec3(0.02, 0.12, 0.08);
    const vec3 uColorB = vec3(0.10, 0.80, 0.50);
    const vec3 uColorC = vec3(0.90, 0.80, 0.20);

    void main() {
        vec2 uvCoord = gl_FragCoord.xy / uResolution.xy;
        ...
        for (float i = 1.0; i < 7.0; i++){
            uv.x += 0.15 / i * cos(i * 2.0 * uv.y + t + mouse.x * uMouseStrength);
            uv.y += 0.15 / i * cos(i * 2.0 * uv.x + t + mouse.y * uMouseStrength);
        }
        
        vec2 diff = (uvCoord - uMouse);
        float dist = length(diff);
        float falloff = exp(-dist * 15.0);
        float ripple = sin(25.0 * dist - t * 5.0) * 0.05 * uMouseStrength;
        uv += (diff / (dist + 0.0001)) * ripple * falloff;
        
        vec3 color = mix(uColorA, uColorB, abs(sin(t - uv.y - uv.x)));
        color = mix(color, uColorC, abs(cos(t * 0.8 + uv.x * 2.5)));
        
        float highlight = 0.1 / (abs(sin(t - uv.y - uv.x)) + 0.1);
        color += uColorB * highlight * 0.5;
        
        color *= 1.0 - length(uvCoord - 0.5) * 1.2;
...
export const ChronoRippleHero = ({ className = '', children, ...props }: any) => (
